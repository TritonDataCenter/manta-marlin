/*
 * tst.msplit.js: msplit tests
 */

var mod_assert = require('assert');
var mod_fs = require('fs');
var mod_child_process = require('child_process');

var mod_bunyan = require('bunyan');
var mod_vasync = require('vasync');

var common = require('../common');

var msplit = null;
var tmpdir = process.env['TMPDIR'] || '/tmp';

var log = new mod_bunyan({
    'name': 'tst.msplit.js',
    'level': process.env['LOG_LEVEL'] || 'debug'
});

mod_vasync.pipeline({ 'funcs': [
	findMsplit,
	testNoOpts,
	testBasic,
	testBasicAltField,
	testBasicJson,
	testBasicExec,
	testBasicExecJson,
	testExecWithoutReturn,
	testExecWithTrailingSemicolon
] }, function (err) {
	if (err) {
		log.fatal(err, 'test failed');
		throw (err);
	}

	log.info('test passed');
});

function runTest(opts, callback)
{
	var spawn = mod_child_process.spawn(msplit, opts.opts);
	var pid = spawn.pid;

	var stdout = '';
	spawn.stdout.on('data', function (data) {
		stdout += data;
	});

	var stderr = '';
	spawn.stderr.on('data', function (data) {
		stderr += data;
	});

	var error = null;
	spawn.on('error', function (err) {
		error = err;
	});

	spawn.stdin.on('error', function (err) {
		error = err;
	});

	spawn.on('close', function (code) {
		var files = {};
		if (!error) {
			for (var i = 0; i < opts.nReducers; ++i) {
				// Must match what's in msplit's msTmpFilePrefix
				var n = '/var/tmp/msplit.' + pid +
					'.' + i;
				files[i] = mod_fs.readFileSync(n, 'utf8');
			}
		}
		var result = {
			stdout: stdout,
			stderr: stderr,
			files: files,
			code: code,
			error: error
		};
		callback(result);
	});

	process.nextTick(function () {
		spawn.stdin.write(opts.stdin || '');
		spawn.stdin.end();
	});
}

function findMsplit(_, next)
{
	common.findbin(function (err, bin) {
		if (err) {
			next(err);
			return;
		}
		msplit = bin + '/msplit';
		next();
	});
}

function testNoOpts(_, next)
{
	log.info('running testNoOpts');
	runTest({
		stdin: '',
		opts: []
	}, function (result) {
		mod_assert.equal(2, result.code);
		next();
	});
}

function testBasic(_, next)
{
	var sin = '1\n2\n3\n4\n';
	log.info('running testBasic');
	runTest({
		stdin: sin,
		opts: ['-t', '-n', 2],
		nReducers: 2
	}, function (result) {
		mod_assert.equal(0, result.code);
		mod_assert.deepEqual({
				'0': '1\n3\n',
				'1': '2\n4\n'
		}, result.files);
		mod_assert.ok(result.error === null);
		next();
	});
}

function testBasicAltField(_, next)
{
	var sin = ['1,2,3',
		    '1,2,2',
		    '1,1,1',
		    '1,3,4'].join('\n');

	log.info('running testBasicAltField');
	runTest({
		stdin: sin,
		opts: ['-t', '-n', 2, '-f', '3', '-d', ','],
		nReducers: 2
	}, function (result) {
		mod_assert.equal(0, result.code);
		mod_assert.deepEqual({
				'0': '1,2,3\n1,1,1\n',
				'1': '1,2,2\n1,3,4\n'
		}, result.files);
		mod_assert.ok(result.error === null);
		next();
	});
}

function testBasicJson(_, next)
{
	var sin = '{"x":1}\n{"x":2}\n{"x":3}\n{"x":4}\n';
	log.info('running testBasicJson');
	runTest({
		stdin: sin,
		opts: ['-t', '-n', 2, '-j', '-f', 'x'],
		nReducers: 2
	}, function (result) {
		mod_assert.equal(0, result.code);
		mod_assert.deepEqual({
				'0': '{"x":1}\n{"x":3}\n',
				'1': '{"x":2}\n{"x":4}\n'
		}, result.files);
		mod_assert.ok(result.error === null);
		next();
	});
}

function testBasicExec(_, next)
{
	var sin = '1\n2\n3\n4\n';
	log.info('running testBasicExec');
	runTest({
		stdin: sin,
		opts: ['-t', '-n', 2, '-e',
		       'return (line < 3 ? "1" : "2")'],
		nReducers: 2
	}, function (result) {
		mod_assert.equal(0, result.code);
		mod_assert.deepEqual({
				'0': '1\n2\n',
				'1': '3\n4\n'
		}, result.files);
		mod_assert.ok(result.error === null);
		next();
	});
}

function testBasicExecJson(_, next)
{
	var sin = '{"x":1}\n{"x":2}\n{"x":3}\n{"x":4}\n';
	log.info('running testBasicExecJson');
	runTest({
		stdin: sin,
		opts: ['-t', '-n', 2, '-j', '-e',
		       'this.x < 3 ? "1" : "2"'],
		nReducers: 2
	}, function (result) {
		mod_assert.equal(0, result.code);
		mod_assert.deepEqual({
				'0': '{"x":1}\n{"x":2}\n',
				'1': '{"x":3}\n{"x":4}\n'
		}, result.files);
		mod_assert.ok(result.error === null);
		next();
	});
}

function testExecWithoutReturn(_, next)
{
	var sin = '1\n2\n3\n4\n';
	log.info('running testExecWithoutReturn');
	runTest({
		stdin: sin,
		opts: ['-t', '-n', 2, '-e',
		       'line < 3 ? "1" : "2"'],
		nReducers: 2
	}, function (result) {
		mod_assert.equal(0, result.code);
		mod_assert.deepEqual({
				'0': '1\n2\n',
				'1': '3\n4\n'
		}, result.files);
		mod_assert.ok(result.error === null);
		next();
	});
}

function testExecWithTrailingSemicolon(_, next)
{
	var sin = '1\n2\n3\n4\n';
	log.info('running testExecWithTrailingSemicolon');
	runTest({
		stdin: sin,
		opts: ['-t', '-n', 2, '-e',
		       'line < 3 ? "1" : "2";'],
		nReducers: 2
	}, function (result) {
		mod_assert.equal(0, result.code);
		mod_assert.deepEqual({
				'0': '1\n2\n',
				'1': '3\n4\n'
		}, result.files);
		mod_assert.ok(result.error === null);
		next();
	});
}

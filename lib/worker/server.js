...
function main()
{
	...
	moray_args = {
	    'log': log,
	    'host': conf['moray']['storage']['host'],
	    'port': conf['moray']['storage']['port'],
	...
	log.info('using RemoteMoray at host %s port %d',
	    moray_args['host'], moray_args['port']);
	moray = new mod_moray.RemoteMoray(moray_args);

	conf.log = log.child({ 'component': 'locator' });
	...
	worker = mwWorkerInstance = new mod_worker.mwWorker({
	    ...
	    'locator': locator,
	    'log': log,
	    'locatePerTick': conf['locatePerTick'],
...
function mwKangGetObject(type, ident)
{
	...
	obj = mwWorkerInstance.mw_jobs[ident];
	return ({
	    'job': obj.js_job,
	    'state': obj.js_state,
	    'state_time': obj.js_state_time,
	    'pending_start': obj.js_pending_start,
	    'changed': obj.js_changed,
	    'groups_tosave': obj.js_groups_tosave,
	    'location_queries': obj.js_location_queries,
	    'location_results': obj.js_location_results
	});
}

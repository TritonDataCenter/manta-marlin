...
function MantaLocator(indexconf, log, moray_bus, moray_bus_options)
{
	...
	this.ml_storage_map = {};
	this.ml_server_map = {};
	this.ml_bus = moray_bus;
	this.ml_bus_options = moray_bus_options;
	mod_events.EventEmitter();

	var loc = this;
	var ringReady = false;
	var busReady = false;
	function testEmitReady() {
		if (ringReady && busReady) {
	...
	/*
	 * Refresh the maps every 5 minutes
	 */
	function refreshPeriodically() {
		setTimeout(function () {
			loc.refreshMantaStorageMap(function () {
				refreshPeriodically();
			});
		}, 360000);
	}
	...
	this.ml_bus.once('ready', function () {
		loc.refreshMantaStorageMap(function () {
			busReady = true;
			testEmitReady();
			refreshPeriodically();
...
MantaLocator.prototype.refreshMantaStorageMap = function (callback) {
	var loc = this;
	var newStorageMap = {};
	var newServerMap = {};
	this.ml_bus.oneshot(
		...
		function (rec) {
			var v = rec['value'];
			newStorageMap[v['manta_storage_id']] = v;
			newServerMap[v['server_uuid']] = v;
		},
		function () {
			loc.ml_storage_map = newStorageMap;
			loc.ml_server_map = newServerMap;
			callback();
		});
};
...
function populateSharkData(locator, result, shark) {
	var rec = locator.ml_storage_map[shark['manta_storage_id']] ||
		locator.ml_server_map[shark['server_uuid']];
	return ({

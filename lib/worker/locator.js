...
MantaLocator.prototype.locate = function (keys, callback)
{
	...
	this.ml_ops[uuid] = mod_vasync.forEachParallel({
		...
		ring.getMetadata(key, subcallback);
	    }
	}, function (err, rv) {
		delete (ops[uuid]);
		 ...
		 * XXX check format of data here.
		 * XXX check that 404 != an error for these purposes.
		 */
		console.log(rv['successes']);

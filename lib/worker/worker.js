...
function Worker(args)
{
	...
	/*
	 * In order to deal with a corner case when cancelling jobs, we need to
	 * keep track of cancelled jobs even after they've finished.  The value
	 * in this object is either "checking" (in which case a job *may* be
	 * cancelled, but we don't know yet) or a throttler that indicates when
	 * we last performed a server-side update of the job records.  See
	 * onRecordForUnknownJob for details.
	 */
	this.w_jobs_cancelled = {};		/* cache of cancelled jobs */
}
...
Worker.prototype.onRecordForUnknownJob = function (record)
{
	 ...
	 * have timeJobCancelled set.  We always log an error for this case, but
	 * if the job really is cancelled, we also trigger a server-side update
	 * to set timeJobCancelled on these records to avoid catching this one
	 * in the future.
	 *
	...
	if (record['bucket'] == this.w_buckets['jobinput'] ||
		...
		if (!this.w_jobs_cancelled.hasOwnProperty(jobid)) {
			this.w_log.error('onRecord: dropping record for ' +
			    'possibly cancelled job (initiating check)',
			    record);
			jc = new JobCancelState();
			jc.jc_checking = true;
			this.w_jobs_cancelled[jobid] = jc;
			this.w_bus.oneshot(this.w_buckets['job'],
				...
				if (rec['value']['timeCancelled']) {
					...
					/*
					 * We could negative-cache to avoid
					 * rechecking this too often, but this
					 * is by definition a bug, and we'll be
					 * logging errors at this point, and the
					 * bus will throttle these queries
					 * anyway.
					 */
					jc.jc_checking = false;
					delete (worker.w_jobs_cancelled[jobid]);
				}
			    });
		} else if (this.w_jobs_cancelled[jobid].jc_checking) {
			if (!dologthrottle)
				this.w_log.warn('onRecord: dropping record ' +
				    'for possibly cancelled job (check ' +
				    'pending)', record);
		} else {
			if (!dologthrottle)
				this.w_log.warn('onRecord: dropping record ' +
		...
		this.w_log.error('onRecord: dropping record for unknown job',
		    record);
	}
...
Worker.prototype.jobCancelRecords = function (jobid)
{
	...
	jc = this.w_jobs_cancelled[jobid];

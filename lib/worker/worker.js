...
Worker.prototype.initPoll = function ()
{
	...
	this.w_init_pipeline = mod_vasync.pipeline({
		...
		function fetchMantaStorage(_, callback) {
			var queryconf = wQueries.wqMantaStorage;
			var query = queryconf['query'];
			var bucket = worker.w_buckets[queryconf['bucket']];
			worker.w_bus.oneshot(bucket, query,
			    worker.w_bus_options,
...
Worker.prototype.onRecordTask = function (record, barrier)
{
	...
	if (record['value']['state'] != 'done') {
		...
		this.taskRecordCleanup(record, barrier, job, now);
    ...
    now)
{
	...
	if (record['value']['phaseNum'] > 0 &&
		...
		if (phase.p_type == 'reduce') {
			job.j_ndeletes += record['value']['nInputs'];
...
Worker.prototype.taskRetryReduce = function (record, barrier, job, phase, now)
{
	...
	var server = mod_jsprim.randElt(Object.keys(this.w_agents));
	var agent = this.w_agents[server];
	...
	value['server'] = server;
	value['serverGeneration'] = agent.a_record['value']['generation'];
	value['mantaComputeId'] = agent.a_record['value']['mantaComputeId'];
	...
	this.w_bus.batch([ [
		...
		'retryServerGeneration': value['serverGeneration'],
...
Worker.prototype.onRecordTaskInput = function (record, barrier)
{
	...
	if (record['value']['wantInputRemoved'] &&
	    record['value']['timeInputRemoved'] === undefined) {
		this.taskRecordCleanup(record, barrier, job, now);
		return;
	}
	...
	value = {
	    ...
	    'server': record['value']['retryServer'],
	    'serverGeneration': record['value']['retryServerGeneration'],
 ...
 */
Worker.prototype.taskRecordCleanup = function (record, barrier, job, now)
{
	var worker = this;
	 ...
	 */
	keep = job.j_job['options'] && job.j_job['options']['keepIntermediate'];
	if (keep || !keyIsAnonymous(record['value']['input'], job.j_id)) {
		this.taskRecordCleanupFini(record, barrier, job, now, null);
		return;
	}
	...
	this.w_deletes_out.push({
		...
		now = mod_jsprim.iso8601(new Date());
		worker.taskRecordCleanupFini(record, barrier, job, now, err);
	    }
	});
};

Worker.prototype.taskRecordCleanupFini = function (record, barrier, job, now,
    delete_err)
{
	/*
	...
	this.w_bus.putBatch([ [
		...
		/*
		 * There's no reason this should be able to fail, since
		 * transient errors will be retried and nothing else should be
		 * updating the record concurrently to create a conflict.
		 */
		mod_assert.ok(!err);
...
{
	var v = newrec['value'];
	this.w_storage_map['by_storage_id'][v['manta_storage_id']] = v;
	this.w_storage_map['by_zone_uuid'][v['zone_uuid']] = v;
};
...
Worker.prototype.jobAssigned = function (job)
{
	 ...
	 * serverGeneration values are dealt with.  We don't have to finish
	...
	mod_jsprim.forEachKey(this.w_agents, function (instance, agent) {
		var filter = sprintf('(!(serverGeneration=%s))',
	...
	job.j_phases.forEach(function (phase, pi) {
		...
		phase.p_reducers.forEach(function (reducer, ri) {
			...
			value['serverGeneration'] = worker.w_agents[
			    server].a_record['value']['generation'];
			value['mantaComputeId'] = worker.w_agents[
			    server].a_record['value']['mantaComputeId'];
...
Worker.prototype.jobTick = function (job)
{
	...
	if (this.jobDone(job)) {
		...
		this.w_bus.unsubscribe(job.j_subscrips['tasks needing retry']);
		delete (job.j_subscrips['tasks needing retry']);

		this.w_bus.unsubscribe(job.j_subscrips['taskinputs retry']);
		delete (job.j_subscrips['taskinputs retry']);

		this.w_bus.unsubscribe(
		    job.j_subscrips['taskinputs needing delete']);
		delete (job.j_subscrips['taskinputs needing delete']);

		this.w_bus.unsubscribe(
		    job.j_subscrips['tasks needing delete']);
		delete (job.j_subscrips['tasks needing delete']);
 ...
 * with a given serverGeneration concurrently with having discovered that that
 * agent has restarted (and its serverGeneration changed).  Without this check,
 * we might never update these records.
 */
Worker.prototype.taskPostDispatchCheck = function (task)
{
	var taskvalue, server, agent;
	var job, filter, timestamp;

	taskvalue = task.t_value;
	server = taskvalue['server'];
	agent = this.w_agents[server];
	mod_assert.ok(agent !== undefined);

	if (agent.a_record['value']['generation'] ==
	    taskvalue['serverGeneration'])
		return;
	...
	filter = sprintf('(!(serverGeneration=%s))',
	    agent.a_record['value']['generation']);
	timestamp = mod_jsprim.iso8601(new Date());
	this.agentFailTasks(server, job, timestamp, filter);
};
...
{
	var worker = this;
	var which, server, zonename, task, value, origin_value, phase, isretry;
	var locs;
	...
	if (dispatch.d_objectid == '/dev/null' &&
		...
		if (dispatch.d_origin['bucket'] == this.w_buckets['task']) {
			...
			locs = locs.filter(function (loc) {
				return (loc['host'] !=
				    dispatch.d_origin['value']['server']);
			});
	...
	value['mantaComputeId'] = this.w_agents[server].
	    a_record['value']['mantaComputeId'];
	value['agentGeneration'] = this.w_agents[server].
	    a_record['value']['generation'];
...
Worker.prototype.dispReduce = function (dispatch)
{
	...
	value = {
	    ...
	    'serverGeneration': worker.w_agents[
		task.t_value['server']].a_record['value']['generation'],

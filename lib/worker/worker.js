 ...
 * o do we handle cancelled tasks updated by the worker properly?
 * o review XXXs
 * o review error handling on putBatch failures and concurrent putBatches
 * o quiesce: update quiesce() implementation
 * o re-add agent timeouts (remove timeTaskAbandon tunable)
 *   note: worker needs to write out "error" objects for cancelled tasks
 * o reimplement piped jobs
 ...
 *
 * Agent notes:
 * - agent must correctly process zero-input tasks
 * - agent must set p0key on taskoutput records
 */
...
function JobState(args)
{
	...
	if (j['input']) {
		...
		/* TODO Remove maxRecentRequests tunable when I remove this. */
...
Worker.prototype.initPoll = function ()
{
	...
	var query = queryconf['query'].bind(null, worker.w_conf);
...
Worker.prototype.onRecordTask = function (record, barrier)
{
	...
	/*
	 * XXX MANTA-928:
	 * This also needs to server-side batch-update the taskoutput records.
	 * If the task succeeded, this should set committed *and* a new "valid"
	 * field to "true" for all taskoutputs.  If it failed, it should set
	 * timePropagated on those taskoutputs.  It should also set
	 * timePropagated on all last-phase records?
	 */
...
Worker.prototype.jobLoadReduceTask = function (job, record)
{
	...
	reducer.r_task.t_record = record;
...
Worker.prototype.jobError = function (job, pi, code, message, messageInternal)
{
	...
	this.w_bus.putBatch([ [ 'error', uuid, value ] ]);
};
...
Worker.prototype.jobLoaded = function (job)
{
	...
	for (pi = 0; pi < job.j_phases.length; pi++) {
		...
		for (ri = 0; ri < phase.p_reducers.length; ri++) {
			...
			this.w_bus.putBatch([ [ 'task', task.t_id, value ] ]);
...
Worker.prototype.jobSave = function (job)
{
	...
	this.w_bus.putBatch(records, options, function (err) {
...
Worker.prototype.dispLocateResponse = function (dispatches, err, locations)
{
	...
	for (i = 0; i < dispatches.length; i++) {
		...
		dispatch.d_locations = locations[iobjname];
		this.w_locates_in.push(dispatch);

		locations[iobjname].forEach(function (loc) {
...
Worker.prototype.reduceEndInput = function (job, i, now)
{
	...
	phase.p_reducers.forEach(function (reducer) {
		...
		worker.w_bus.putBatch([
		    [ 'task', task.t_id, task.t_value, options ] ], {
				 ...
				 * anyway.)
				 */
				return (mod_bus.mergeRecords([ 'state' ],

...
Worker.prototype.dispResolveUser = function (dispatch)
{
	...
	key = '/login/' + login;
	if (this.w_auths_pending.hasOwnProperty(key)) {
		this.w_log.debug('piggy-backing auth "%s" onto existing ' +
		    'request', key);
		this.w_auths_pending[key].d_auths.push(dispatch);
		return;
	}

	this.w_auths_pending[key] = dispatch;

	this.w_redis.get(key, function (err, data) {
		var record;

		mod_assert.equal(worker.w_auths_pending[key], dispatch);
		delete (worker.w_auths_pending[key]);

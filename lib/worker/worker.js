 ...
 */
var mwConf = {
    'jobsBucket': mod_config.mcBktJobs,
    ...
    /* timeout values */
    'taskGroupIdleTime': 40 * 1000	/* idle time after which tg is dead */
};
...
mwJobState.prototype.loadTaskGroups = function (groups, phasenum)
{
	...
	mod_jsprim.forEachKey(phase.p_dead, function (tgid) {
		...
		group['inputKeys'].forEach(function (key) {
			if (!phase.p_keys_left.hasOwnProperty(key))
				return;
...
mwJobState.prototype.onTaskGroup = function (taskgroups)
{
	 ...
	 */
	this.js_log.info('updating task groups for phase %s', this.js_phasei);
...
mwJobState.prototype.finish = function ()
{
	 ...
	 */
	this.js_log.info('cancelling groups', this.js_deadgroups.map(

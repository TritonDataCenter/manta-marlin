...
var mwPollConfig = {
    'job':		{ 'mkfilter': pfJob		},
    'jobinput': 	{ 'mkfilter': pfJobInput	},
    'task':		{ 'mkfilter': pfTask		},
    'taskinput':	{ 'mkfilter': pfTaskInput	},
    'taskoutput':	{ 'mkfilter': pfTaskOutput 	}
};

function pfJob(worker, last_mtime)
{
	return (worker.w_moray.filterJobs(
	    worker.w_uuid, worker.w_time_job_abandon, last_mtime));
}

function pfJobInput(worker, _, last_id)
{
	var watch;
...
function pfTask(worker, last_mtime)
{
	var watch = [];
	...
	return (worker.w_moray.filterTasks(watch, last_mtime));
}

function pfTaskInput(worker, _, last_id)
{
	var watch = [];
...
function pfTaskOutput(worker, _, last_id)
{
	var watch = [];

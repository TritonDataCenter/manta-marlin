...
/*
 * Assignment of reduce tasks is quite simple: we keep track of all hosts we've
 * ever seen and assign reduce tasks randomly to them.
 */
var mwKnownHosts = {};
 ...
 *
 *    locatePerTick	max number of locate requests issued per job per tick
 *
 ...
 *
 *    taskGroupInterval	how frequently to check for task group updates
 *
...
function mwWorker(args)
{
	...
	mod_assert.equal(typeof (args['locatePerTick']), 'number');
	...
	this.mw_tg_interval = args['taskGroupInterval'];
	this.mw_tg_idle_time = args['taskGroupIdleTime'];
	this.mw_maxlocate = args['locatePerTick'];
...
mwWorker.prototype.onJob = function (metadata, job)
{
	...
	var newjob = new mwJobState({
	    ...
	    'taskgroup_interval': this.mw_tg_interval,
	    'taskgroup_idle_time': this.mw_tg_idle_time,
	    'maxlocate': this.mw_maxlocate
	});
...
function mwJobState(args)
{
	...
	mod_assert.equal(typeof (args['maxlocate']), 'number');
	mod_assert.equal(typeof (args['save_interval']), 'number');
	mod_assert.equal(typeof (args['taskgroup_interval']), 'number');
	...
	this.js_maxlocate = args['maxlocate'];

	/* helper objects */
	this.js_locator = args['locator'];
	this.js_moray = args['moray'];
	this.js_worker = args['worker'];
	this.js_log = args['log'];
	this.js_throttle = new mod_mautil.Throttler(args['taskgroup_interval']);
	...
	this.js_phases = this.js_job['phases'].map(function (jobphase) {
		return ({
		    ...
		    p_ngroups: 0	/* number of assigned task groups */
		});
	});
...
mwJobState.prototype.tick = function ()
{
	...
	if (!this.js_throttle.tooRecent()) {
		this.js_throttle.start();
		this.js_moray.watchTaskGroups(this.js_jobid,
		    this.js_ontaskgroup);
		this.js_throttle.done();
	}
};
...
/*
 * Collect all unassigned keys and mark the first phase in which each one is
 * used.
 */
mwJobState.prototype.collectUnassignedKeys = function (limit)
{
	var rv, nkeys, pi, phase, key;

	rv = {};
	nkeys = 0;

	for (pi = 0; pi < this.js_phases.length && nkeys < limit; pi++) {
		phase = this.js_phases[pi];

		if (this.js_job['phases'][pi]['type'] == 'generic')
			continue;
...
mwJobState.prototype.runLocateUnassignedKeys = function ()
{
	...
	if (this.js_location_queries !== undefined ||
	    this.js_location_results !== undefined ||
	    !mod_jsprim.isEmpty(this.js_groups_tosave) ||
	    !mod_jsprim.isEmpty(this.js_groups_saving))
		return;

	keyphases = this.collectUnassignedKeys(this.js_maxlocate);
...
/*
 * Runs through unassigned generic keys as well as the results of a previous
 * "locate" operation for map keys and uses the results to assign keys to new
 * task groups.
 */
mwJobState.prototype.runAssignKeys = function ()
{
	this.assignMapKeys();
	this.assignGenericKeys();
};

/*
 * Assign a task group for the first "generic" or "reduce" phase that's ready.
 * There cannot be more than one that's ready, since the later one would depend
 * on the previous one having completed.
 */
mwJobState.prototype.assignGenericKeys = function ()
{
	var pi, jobphase, phase, pphase;
	var hosts, host, group, key;

	for (pi = 0; pi < this.js_phases.length; pi++) {
		jobphase = this.js_job['phases'][pi];
		phase = this.js_phases[pi];

		if (jobphase['type'] == 'storage-map')
			continue;

		if (pi > 0) {
			/*
			 * Don't assign the task group until all keys in the
			 * previous phase have completed.  XXX this can be
			 * removed when we have streaming taskgroups.
			 */
			pphase = this.js_phases[pi - 1];

			if (!mod_jsprim.isEmpty(pphase.p_keys_unasgn) ||
			    !mod_jsprim.isEmpty(pphase.p_keys_left))
				return;
		}

		if (phase.p_ngroups === 0)
			break;
	}

	if (pi == this.js_phases.length)
		return;
	...
	/*
	 * We assign generic and reduce phases to any host that we know about.
	 * In parallel, we locate input keys for these phases, just like we
	 * would for map phases.  The very first time through here, we may not
	 * yet know about any hosts, and we won't be able to schedule the job,
	 * but as soon as we locate any of its keys, we'll have a host we can
	 * use.
	 *
	 * Right now, we locate all of the keys of a reduce phase even though we
	 * don't do anything with the results except populate mwKnownHosts, and
	 * we only need one result to do that.  But in the future we likely want
	 * to locate each key before dispatching it to the reducer anyway, if
	 * for no other reason than to resolve a canonical objectid for it.
	 * XXX So it's not worth addressing this now, given that this will all
	 * change once we can stream input keys to reducers.
	 */
	if (mod_jsprim.isEmpty(mwKnownHosts)) {
		this.js_log.warn('no hosts available yet to run generic phase');
		return;
	}

	hosts = Object.keys(mwKnownHosts);
	host = hosts[Math.floor(Math.random() * hosts.length)];
	group = this.newTaskGroup(host, pi);

	for (key in phase.p_keys_unasgn) {
		mod_assert.ok(phase.p_keys_unasgn.hasOwnProperty(key));
		delete (phase.p_keys_unasgn[key]);
		phase.p_keys_left[key] = true;
		group['inputKeys'].push({ 'key': key });
	}

	this.js_groups_tosave[group['taskGroupId']] = group;
};

/*
 * Go through the most recent group of located keys and assign them to map
 * task groups.
 */
mwJobState.prototype.assignMapKeys = function ()
{
	var newgroups, groupforzone, key, loc, pi, phase, copy, group;
	...
	for (key in this.js_location_results) {
		...
		loc.forEach(function (l) { mwKnownHosts[l['host']] = true; });

		if (this.js_job['phases'][pi]['type'] != 'storage-map')
			/*
			 * Task groups for these phases are assigned by
			 * assignGenericKeys above.
			 */
			continue;
		...
		if (!groupforzone.hasOwnProperty(copy['zonename'])) {
			group = this.newTaskGroup(copy['host'], pi);
			newgroups[group['taskGroupId']] = group;
			groupforzone[copy['zonename']] = group;
		} else {
...
mwJobState.prototype.newTaskGroup = function (host, pi)
{
	this.js_phases[pi].p_ngroups++;

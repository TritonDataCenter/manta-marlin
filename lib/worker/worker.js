 ...
 *
 * XXX Currently these all use the cache because that's the only way we have to
 * avoid repeating records, even for immutable records like job inputs.
 * Instead, it would be nice to be able to encode in the query the bucket's
 * generation count or something like that.
 */
var mwPollConfig = {
    'job':		{ 'cache': true,   'mkfilter': pfJob		},
    'jobinput': 	{ 'cache': true,   'mkfilter': pfJobInput	},
    'task':		{ 'cache': true,   'mkfilter': pfTask		},
    'taskinput':	{ 'cache': true,   'mkfilter': pfTaskInput	},
    'taskoutput':	{ 'cache': true,   'mkfilter': pfTaskOutput	}
};
 ...
 */
function WorkerJobTask(jobid, pi, timeDispatched)
{
...
Worker.prototype.onRecord = function (record, changed)
{
	...
	this.w_log.debug('record: "%s" "%s" etag %s (changed: %j)',
	    name, record['key'], record['_etag'], changed);
...
Worker.prototype.jobRemove = function (job)
{
	...
	this.w_cache.remove(this.w_buckets['job'], job.j_id);
...
Worker.prototype.jobLoad = function (job)
{
	...
	delete (job.j_init_records['jobinput']);
...
Worker.prototype.jobDone = function (job)
{
	...
	if (job.j_locates > 0 || job.j_queued > 0)
		return (false);
...
Worker.prototype.enqueue = function (name, key, value, options)
{
	var bucket, c;
...
Worker.prototype.recordSave = function (ent)
{
	...
	this.w_moray.putObject(bucket, key, value, options, function (err) {
		...
		worker.w_log.info('saved "%s" "%s"', bucket, key);
	});
};
...
Worker.prototype.taskCreate = function (job, pi, now)
{
	...
	var task = new WorkerJobTask(job.j_id, pi, now);
...
Worker.prototype.keyLocate = function (key, ent)
{
	...
	this.w_log.info('enqueued locate request for "%s" (for job %s)',
	    key, job.j_id);
};
...
Worker.prototype.keyDispatch = function (source, loc, now)
{
	...
	if (source['bucket'] == this.w_buckets['jobinput']) {
		...
		mod_assert.ok(pi < job.j_phases.length);
	}
...
Worker.prototype.keyDispatchReduce = function (job, pi, loc, now, err)
{
	...
	/*
	 * XXX if !isnew && this.writePending(), we need to queue
	 * another write here.
	 */
	if (!this.writePending('task', task.t_id))
		this.enqueue('task', task.t_id, task.t_value);

...
function mwJobState(args)
{
	...
	for (var i = 0; i < this.js_job['phases'].length; i++) {
		...
		/*
		 * For any completed phases, we store into p_results whatever
		 * results we already have.  The only way we know whether phase
		 * i has completed is because i + 1 also has some results.
		 * This algorithm still allows a phase to appear to go
		 * "backwards" because some results are lost, but only
		 * transiently (since we will immediately see the task results
		 * and restore them).  This logic will have to be revisited for
		 * streaming anyway.
		 */
		if (this.js_job['phaseResults'] &&
		    i + 1 < this.js_job['phases'].length &&
		    this.js_job['phaseResults'][i + 1].length > 0)
			results = this.js_job['phaseResults'][i];
		else
			results = [];
...
mwJobState.prototype.jobRestore = function ()
{
	...
	this.js_moray.listTaskGroups(jobid, function (err, groups) {
		...
		/*
		 * After loading the task groups, we may have jumped directly to
		 * the "running" stage.  But if not, move to planning now.
		 */
		if (job.js_state == 'uninitialized')
...
mwJobState.prototype.loadTaskGroups = function (groups, phasenum)
{
	...
	groups.forEach(function (groupent) {
		...
		if (groupent['etag'] == phase.p_etag[tgid] &&
		    groupent['mtime'] == phase.p_mtime[tgid])
			return;

		phase.p_groups[tgid] = group;
		phase.p_etag[tgid] = groupent['etag'];
		phase.p_mtime[tgid] = groupent['mtime'];
	...
	groups.forEach(function (groupent) {
		...
		if (group['state'] == 'done' &&
		    group['results'].length != group['inputKeys'].length) {
			job.js_log.error('task group %s failed prematurely',
			    group['taskGroupId']);
			job.js_phases[group['phaseNum']].p_dead[
			    group['taskGroupId']] = 'EJ_UNKNOWN';
		}
	});

	mod_jsprim.forEachKey(phase.p_dead, function (tgid, code) {
		...
		group['inputKeys'].forEach(function (key) {
			...
			phase.p_results.push({
			    ...
			    'error': {
				'code': code,
				'message': code == 'EJ_TIMEOUT' ?
				    'system timeout' : 'internal error'
			    }
			});
...
mwJobState.prototype.taskGroupAssign = function ()
{
	...
	mod_assert.ok(this.js_pending_start === undefined);
	this.js_pending_start = new Date();
	this.js_locator.locate(unassigned, function (err, locs) {
		if (err) {
			job.js_pending_start = undefined;
		...
		for (key in locs) {
			...
			copy = locs[key][
			    Math.floor(Math.random() * locs[key].length)];

			if (!groups.hasOwnProperty(copy['zonename'])) {
				groups[copy['zonename']] = {
				    'jobId': jobid,
			...
			groups[copy['zonename']]['inputKeys'].push({
			    'key': key,
...
mwJobState.prototype.advancePhase = function ()
{
	...
	if (++this.js_phasei < this.js_phases.length) {
		this.preparePhase();
		this.transition(undefined, 'planning');
		return;
	}
	...
	this.js_phases.forEach(function (phasei) {
		mod_jsprim.forEachKey(phasei.p_dead, function (tgid) {
			var group = phasei.p_groups[tgid];

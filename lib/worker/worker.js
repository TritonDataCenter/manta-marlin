 ...
 * used.
 */
mwJobState.prototype.collectUnassignedKeys = function (formap, limit)
{
	var rv, nkeys, pi, jobphase, phase, pphase, key;
	...
	for (pi = 0; pi < this.js_phases.length && nkeys < limit; pi++) {
		...
		if (formap && jobphase['type'] != 'storage-map')
			continue;
		...
		/*
		 * If this is a non-map phase (e.g., "generic" or "reduce"),
		 * then we won't collect any keys for phases whose previous
		 * phase is not yet complete because we can't dispatch these
		 * phases in piecemeal.
		 */
		if (!formap && pi > 0) {
...
mwJobState.prototype.runLocateUnassignedKeys = function ()
{
	...
	keyphases = this.collectUnassignedKeys(true, this.js_maxlocate);
...
/*
 * Assign task groups for keys in "generic" and "reduce" phases, whose tasks are
 * not colocated with storage.
 */
mwJobState.prototype.assignGenericKeys = function ()
{
	var generickeys, hosts;
	var host, group, phase;
	var key, pi;

	generickeys = this.collectUnassignedKeys(false, Infinity);

	if (mod_jsprim.isEmpty(generickeys))
		return;

	hosts = Object.keys(mwKnownHosts);
	if (hosts.length === 0) {
		this.js_log.warn('no hosts available to run generic phase');
		return;
	}
	...
	for (key in generickeys) {
		mod_assert.equal(generickeys[key], pi);

 ...
 *
 * XXX MANTA-928 TODO:
 * o implement MorayBus.putBatch()
 *     o bail out after too many EtagConflict errors, and call jobRemove
 *     o will need to fetch objects and "merge" them for etagconflict errors,
 *       at least for job records and reduce tasks
 * o review XXXs
...
Worker.prototype.jobError = function (job, pi, code, message)
{
	...
	this.w_bus.putBatch([ [ 'error', uuid, {
	    'errorId': uuid,
	    'jobId': job.j_id,
	    'phaseNum': pi,
	    'errorCode': code,
...
Worker.prototype.dispMap = function (dispatch)
{
	...
	this.w_bus.putBatch([ [
		...
		/* XXX MANTA-928 what to do with etags? */
...
Worker.prototype.dispReduce = function (dispatch)
{
	...
	this.w_bus.putBatch([ [
	    ...
	    dispatch.d_origin['key'],
	    origin_value,
	    /* XXX this etag should *never* get EtagConflict */
	    { 'etag': dispatch.d_origin['_etag'] }
	] ], function (err, etags) {
		/* XXX MANTA-928 what to do with etags? */
		/* XXX make sure putBatch logs errors */
		dispatch.d_barrier.done(dispatch.d_id);
	});
...
Worker.prototype.reduceEndInput = function (job, i, now)
{
	...
	phase.p_reducers.forEach(function (reducer) {
		...
		/*
		 * XXX consider all possible cases (agent writes "accept" first,
		 * or it doesn't).
		 */

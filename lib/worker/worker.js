...
function WorkerJobState(args)
{
	 ...
	 * this job), the source job's last-phase, committed "task" records
	 * (which include output references), and task output records associated
	 * with any last-phase, committed task records.
	 */
	if (j['input']) {
		this.j_source = j['input'].substr(
		    j['input'].lastIndexOf('/') + 1);
		this.j_source_job = undefined;
...
Worker.prototype.tick = function ()
{
	...
	mod_jsprim.forEachKey(this.w_agents, function (host, agent) {
		...
		mod_jsprim.forEachKey(agent.a_tasks, function (taskid) {
			ntasks++;
			worker.taskTimedOut(worker.w_tasks[taskid],
			    mod_jsprim.iso8601(now));
		});
...
Worker.prototype.poll = function (now)
{
	...
	mod_jsprim.forEachKey(this.w_jobs, function (jobid, job) {
		...
		mod_jsprim.forEachKey(job.j_polls, function (name, state) {
			var bucket, filter, onrecord, ondone;

			/*
			 * This is a bit kludgy, but for piped jobs, we
			 * interpose on the "jobinput" poll and replace it with
			 * a "task" poll on the source job's committed
			 * last-phase tasks.  The onSearchDone logic is the
			 * same, as long as we propagate the lie.
			 */
			if (name == 'jobinput' && job.j_source) {
				if (!job.j_source_job)
					return;

				var sourcejob = job.j_source_job['value'];

				bucket = worker.w_buckets['task'];
				filter = sprintf('(&(jobId=%s)(phaseNum=%d)' +
				    '(timeCommitted=*)(result=ok)' +
				    '(nOutputs>=1)%s)', job.j_source,
				    sourcejob['phases'].length - 1,
				    state.filter());
				onrecord = function (record) {
					if (state.record(record))
						worker.pipeJobTaskUpdated(
						    job, record);
				};
				ondone = function (_, start, count) {
					worker.onSearchDone(job,
					    worker.w_buckets['jobinput'],
					    start, count);
				};
			} else {
				bucket = worker.w_buckets[name];
				filter = sprintf('(&(jobId=%s)%s)',
				    jobid, state.filter());
				onrecord = function (record) {
					if (state.record(record))
						worker.onRecord(record);
	...
	/* Kick off various polls for jobs with piped input. */
	mod_jsprim.forEachKey(this.w_jobs_piped, function (jobid) {
		var job, jstate, filter;

		job = worker.w_jobs[jobid];
		mod_assert.ok(job);
		mod_assert.ok(job.j_source);

		/* See if we need an updated job record. */
		jstate = job.j_source_poll_job;
		filter = sprintf('(&(jobId=%s)%s)', job.j_source,
		    jstate.filter());
		...
		req = mod_mamoray.poll({
		    ...
		    'now': now,
		    'log': job.j_log,
		    'throttle': jstate.p_throttle,
		    'bucket': worker.w_buckets['job'],
		    'filter': filter,
		    'onrecord': worker.pipeJobSourceUpdated.bind(worker, job)
		});

		jstate.request(filter, req);

		/* Poll for new taskoutputs */
		mod_jsprim.forEachKey(job.j_source_tasks,
		    function (taskid, state) {
			filter = sprintf('(&(taskId=%s)%s)', taskid,
			    state.filter());
			...
			req = mod_mamoray.poll({
			    ...
			    'log': job.j_log,
			    'throttle': state.p_throttle,
			    'bucket': worker.w_buckets['taskoutput'],
			    'filter': filter,
			    'onrecord': worker.pipeJobTaskOutputReceived.bind(
				worker, job, state)
			});
...
Worker.prototype.onRecord = function (record)
{
	...
	if (job.j_dropped) {
		job.j_log.info('onRecord: ignoring record for dropped job',
		    record);
		return;
	}
...
Worker.prototype.jobCreate = function (record)
{
	...
	var j = this.w_jobs[job['jobId']] = new WorkerJobState({
	    'conf': this.w_conf,
...
Worker.prototype.jobUpdate = function (record)
{
	...
	if (!job.j_source && job.j_input_done === undefined &&
...
Worker.prototype.jobRemove = function (job)
{
	...
	for (taskid in job.j_tasks) {
		...
		agent = this.w_agents[task.t_value['server']];
...
Worker.prototype.jobDone = function (job)
{
	...
	/*
	 * XXX There's no check here that we've read all expected taskoutput
	 * records, for either normal or piped jobs.
	 */
...
Worker.prototype.pipeJobSourceUpdated = function (job, record)
{
	...
	if (job.j_dropped) {
		job.j_log.warn('ignoring piped job update for dropped job',
		    record);
		return;
	}

	var pollstate = job.j_source_poll_job;
	if (!pollstate.record(record))
		return;

	if (job.j_source != record['value']['jobId']) {
		job.j_log.warn('ignoring job update for non-source job',
		    record);
		return;
	}

	job.j_log.debug('source job update', record['key'], record['_etag']);

	job.j_source_job = record;
	if (job.j_input_done || !record['value']['timeDone'])
		return;

	job.j_input_done = Date.parse(record['value']['timeDone']);
	job.j_log.info('discovered source job done at', job.j_input_done);
...
Worker.prototype.pipeJobTaskUpdated = function (job, record)
{
	...
	if (job.j_source != record['value']['jobId']) {
		job.j_log.warn('ignoring task update for non-source job',
		    record);
		return;
	}
	...
	/*
	 * If there are external outputs for this task, make a note to fetch
	 * them.
	 */
	if (record['value']['nOutputs'] && record['value']['firstOutputs'] &&
	    record['value']['firstOutputs'].length <
	    record['value']['nOutputs'] &&
	    !job.j_source_tasks.hasOwnProperty(record['value']['taskId']))
		job.j_source_tasks[record['value']['taskId']] =
		    new mod_mamoray.PollState(
			this.w_conf['tunables']['timePoll'], 10);

	/*
	 * Cons up a jobinput record for the sink job for each output from the
	 * source job.
	 */
	...
	record['value']['firstOutputs'].forEach(function (out) {
		job.j_queued++;
		worker.w_jobinputs_in.push({
		    'bucket': worker.w_conf['buckets']['jobinput'],
		    'value': {
			'jobId': job.j_id,
			'key': out['key']
		    }
...
Worker.prototype.pipeJobTaskOutputReceived = function (job, state, record)
{
	...
	if (record['value']['error'] || !record['value']['key'])
		return;

	/*
	 * As with each of the firstOutput records in the task record, we cons
	 * up a fake "jobinput" record for the piped job.
	 */
	job.j_queued++;
	this.w_jobinputs_in.push({
	    'bucket': this.w_conf['buckets']['jobinput'],
	    'value': {
		'jobId': job.j_id,
...
Worker.prototype.taskTimedOut = function (task, now)
{
	...
	task.t_value['error'] = {
	    ...
	    'messageInternal': sprintf('agent "%s" timed out at "%s"',
		task.t_value['server'], now)
	};
...
{
	var task, record, server, zonename;
	...
	if (!dispatch.d_error) {
		...
		if (loc.lr_objectid !== null) {
			var which = mod_jsprim.randElt(loc.lr_locations);
			...
			dispatch.d_error = {
			    'code': 'EJ_NORESOURCES',
			    'message': 'no servers available to run ' +
				'task'
			};
		}
	}
	...
	if (dispatch.d_error) {
		...
		record['objectid'] = loc.lr_objectid || '/dev/null';
		record['server'] = server;
		record['zonename'] = zonename;
		this.taskAssignServer(task, server);
	}
...
Worker.prototype.keyDispatchReduce = function (dispatch)
{
	...
	if (phase.p_reducers[ri].r_task === undefined) {
		...
		if (mod_jsprim.isEmpty(this.w_agents)) {
			...
			this.taskAssignServer(task, record['server']);
		}
	}
	...
	for (pi = 0; pi < job.j_phases.length; pi++) {
		...
		if (!phase.p_reducers) {
			if (phase.p_npending > 0)
				/*
				 * A map task is still running.  End-of-stream
				 * will be propagated when it finishes.
				 */
				return;

			/*
			 * Skip over map phases with no pending tasks since
		...
		/*
		 * For reduce phases, check the state of each reduce task.
		 */
		var ntasks = 0;
		var ndone = 0;
		...
		for (ri = 0; ri < phase.p_reducers.length; ri++) {
			...
			if (!task)
				/*
				 * This task has not been dispatched (because no
				 * keys have been assigned to it).  We can
				 * ignore these here because if this phase's
				 * input is done then this task never needs to
				 * be dispatched.
				 */
				continue;
			...
			if (task.t_value['timeInputDone']) {
				/*
				 * If input has already been marked done for
				 * this phase but at least one task is still
				 * running, then there's nothing more to do
				 * until that task completes.
				 */
				if (task.t_value['state'] == 'dispatched' ||
				    task.t_value['state'] == 'running')
					return;
		...
		/*
		 * If there were no reduce tasks at all, then there's no input
		 * for this phase at all and we should continue propagating "end
		 * of input" to the next phase.  (Recall the invariant that if
		 * we get to phase i in this loop, then all phases prior to i
		 * have completely processed all input already.)
		 */
		if (ntasks === 0)
			continue;

		/*
		 * If there were some tasks not marked input-done, then we
		 * haven't fully propagated end-of-input to this phase yet.  Do
		 * so now.  We can also stop iterating here because subsequent
		 * phases may get input from the tasks we're only now marking.
		 */
		if (ndone < ntasks)
			break;

		/*
		 * At this point, we had a bunch of tasks with input marked
		 * done, and which themselves were all done.  That means this
		 * phase is done and we can move on to mark the next phase.
		 */
	}

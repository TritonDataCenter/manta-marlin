...
function WorkerJobPhase(phase)
{
	...
	this.p_anons = [];		/* anonymous keys (reduce only) */
}
...
Worker.prototype.taskCleanup = function (task, job)
{
	...
	/*
	 * For now we store the list of anonymous reduce keys in memory while we
	 * operate on them so that we have the full list of keys that we need to
	 * delete when it finishes.  We only have to do this for non-final
	 * phases.
	 */
	var phase = job.j_phases[task.t_value['phaseNum']];
	if (!phase) {
		this.w_log.warn('taskCleanup: invalid phase number', task);
		return;
	}

	var keys = phase.p_anons;
	phase.p_anons = [];

	this.w_log.debug('taskCleanup: deleting %d keys for task "%s"',
	    keys.length, task.t_id);

	for (var i = 0; i < keys.length; i++) {
		mod_assert.ok(keyIsAnonymous(keys[i], job.j_id));
		this.w_deletes_out.push(keys[i]);
	}
};
...
Worker.prototype.keyDispatch = function (dispatch)
{
	...
	if (!dispatch.d_error && dispatch.d_locate.lr_locations.length === 0) {
		dispatch.d_error = {
		    'code': 'EJ_NOENT',
		    'message': 'no such object: "' + dispatch.d_key + '"'
		};
	}
...
Worker.prototype.keyDispatchReduce = function (dispatch)
{
	...
	if (pi > 0 && keyIsAnonymous(dispatch.d_key, job.j_id))
		job.j_phases[pi].p_anons.push(dispatch.d_key);

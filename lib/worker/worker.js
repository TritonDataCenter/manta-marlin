 ...
 * XXX MANTA-928 TODO:
 * o review XXXs
 * o review error handling on putBatch failures and concurrent putBatches
 * o re-add piggy-backing to auth and locate requests
 * o add caching to auth requests
 * o don't handle redis connection going away transiently
 * o review schema for any housekeeping changes to be made (s/key/{in,out}put/g)
 * o review changes to configuration files:
 *   o propagated to templates and schemas
 *   o any unused values?
 *
 * Agent notes: agent must correctly process zero-input tasks
 */
...
function Worker(args)
{
	...
	this.w_record_handlers = {
	    'health': this.onAgentHealth.bind(this),
	...
	/*
	 * When an input record arrives (either as a "jobinput" or a
	 * "taskoutput" record), we run it through a pipeline that involves
	 * resolving the username, resolving the location, and dispatching the
	 * task.  We manage the state of this request in a "dispatch" object
	 ...
	 *
	 *    d_objname		user-facing object name (includes login)
	 *
	 ...
	 *
	 * During execution of this pipeline we'll fill in the following
	 * additional fields in roughly the following order:
	 *
	 ...
	 *     (see dispLocate)		("host", "zonename" tuples)
	 *
...
Worker.prototype.initMoray = function ()
{
	...
	this.w_bus.initBuckets(this.w_conf['buckets'], function (err) {
		if (err) {
			worker.w_log.fatal(err, 'failed to init buckets');
			throw (err);
		}

		worker.w_log.info('moray buckets created');
		worker.initPoll();
...
Worker.prototype.initPoll = function ()
{
	...
	this.w_init_pipeline = mod_vasync.pipeline({
	    'funcs': [
		function fetchJobs(_, callback) {
			var queryconf = wQueries.wqJobsOwned;
			var query = queryconf['query'].bind(null,
			    worker.w_conf);
			var bucket = worker.w_buckets[queryconf['bucket']];
			var queued = false;
			var count = 0;
			worker.w_bus.oneshot(bucket, query,
			    ...
			    function () {
				worker.w_log.info(
				    'found %d existing jobs', count);
				if (!queued)
					callback();
			    });
		},
	        function fetchHealth(_, callback) {
			var queryconf = wQueries.wqAgentHealth;
			var query = queryconf['query'];
			var bucket = worker.w_buckets[queryconf['bucket']];
			worker.w_bus.oneshot(bucket, query,
			    worker.w_bus_options,
...
Worker.prototype.initPollPost = function ()
{
	...
	queries.forEach(function (queryconf) {
		var options = queryconf['options'] ?
		    queryconf['options'](worker.w_conf) : worker.w_bus_options;
		worker.w_subscrips[queryconf['name']] = worker.w_bus.subscribe(
		    ...
		    options, worker.onRecord.bind(worker));
	});

	this.w_quiesced = 'dummy';
	this.quiesce(false);
};
...
Worker.prototype.onRecordJobInput = function (record, barrier)
{
	...
	dispatch = {
	    ...
	    'd_pi': 0,
	    'd_origin': record,
	    'd_objname': record['value']['key'],
	    'd_p0objname': record['value']['key'],
	    'd_barrier': barrier,
	    'd_ri': undefined,
...
{
	var worker = this;
	var job, phase, now, tochanges, allchanges;

	if ((job = this.jobForRecord(record)) === null)
		return;

	if (record['value']['state'] != 'done' ||
	    record['value']['timeCommitted'] !== undefined) {
		/*
		 * TODO ignore persistently? ditto other places where
		 * we drop a record.
		 */
	...
	if (record['value']['result'] == 'ok') {
		...
		if (record['value']['phaseNum'] == job.j_phases.length - 1) {
			job.j_job['stats']['nJobOutputs'] +=
			    record['value']['nOutputs'];

			tochanges['valid'] = true;
			tochanges['timePropagated'] = now;
		} else {
			phase.p_nunpropagated += record['value']['nOutputs'];
		}
	} else {
		...
		if (record['value']['timeAbandoned'] !== undefined) {
			...
			allchanges.push([ 'put', this.w_buckets['error'],
				...
				'errorCode': 'EJ_INTERNAL',
				'errorMessage': 'internal error',
				'errorMessageInternal': 'agent timed out',
				'input': record['value']['key'],
				'p0input': record['value']['p0key'],
				'prevRecordType': 'task',
	...
	this.w_bus.batch(allchanges, {}, function (err) {
...
Worker.prototype.onRecordTaskOutput = function (record, barrier)
{
	...
	dispatch = {
	    ...
	    'd_origin': record,
	    'd_objname': record['value']['key'],
	    'd_p0objname': record['value']['p0key'],
	    'd_barrier': barrier,
	    'd_ri': undefined,
...
Worker.prototype.onAgentHealth = function (newrec)
{
	...
	if (oldrec['value']['generation'] != newrec['value']['generation']) {
		...
		agent.a_nunchanged = 0;
		this.agentStarted(instance);
		return;
	}

	if (oldrec['_mtime'] != newrec['_mtime']) {
		if (agent.a_nunchanged > 0)
	...
	agent.a_nunchanged++;
	if (agent.a_nunchanged < maxmissed) {
...
Worker.prototype.agentStarted = function (instance)
{
	...
	var filter = sprintf('(!(serverGeneration=%s))', generation);
...
Worker.prototype.agentFailTasks = function (instance, job, timestamp, extra)
{
	...
	filter = sprintf('(&(jobId=%s)(!(state=done))(server=%s)%s)',
	...
	this.w_log.info('agent "%s": job "%s": failing tasks',
	    instance, job.j_id, extra);
	worker.w_bus.batch([
	    [ 'update', bucket, filter, changes ] ], {}, function () {
		worker.w_log.info('agent "%s": job "%s": done failing tasks',
		    instance, job.j_id, extra);
	    });
};
...
Worker.prototype.jobAssigned = function (job)
{
	...
	this.w_bus.oneshot(worker.w_buckets[queryconf['bucket']], query,
	    this.w_bus_options, function (record) {
	        worker.jobLoadReduceTask(job, record);
	    },
...
Worker.prototype.jobLoadReduceTask = function (job, record)
{
	...
	/*
	 * XXX r_ninput needs to be initialized from some known-correct state.
	 * Two options are to store it inside the task record itself and update
	 * that whenever we write a new taskinput (which seems hard to get
	 * right, since it would introduce potentially lots of concurrent writes
	 * to the same record), or else use a bus.count() operation to count the
	 * number at this point during initialization and block transition to
	 * 'running' until we have that number.  Until we do this, reduce tasks
	 * running when a worker crashes may miss inputs.
	 */
...
Worker.prototype.jobError = function (job, pi, code, message, messageInternal)
{
	var uuid = mod_uuid.v4();
	var value = {
	    'errorId': uuid,
...
Worker.prototype.jobLoaded = function (job)
{
	 ...
	 */
	var worker = this;
	var queries = [ wQueries.wqJobTasksDone ];
	var qconf;

	if (!job.j_input_done || job.j_job['timeInputDoneRead'] === undefined) {
		queries.push(wQueries.wqJobInputs);
	} else {
		qconf = wQueries.wqJobInputs;
		worker.w_bus.oneshot(
		    worker.w_buckets[qconf['bucket']],
		    qconf['query'].bind(null, job.j_id),
		    worker.w_bus_options, worker.onRecord.bind(worker),
		    function () {
			job.j_input_fully_read = true;
			worker.jobPropagateEnd(job, null);
			worker.jobTick(job);
	...
	if (job.j_phases.length > 1)
		queries.push(wQueries.wqJobTaskOutputsUnpropagated);

	queries.forEach(function (queryconf) {
		job.j_subscrips[queryconf['name']] = worker.w_bus.subscribe(
		    worker.w_buckets[queryconf['bucket']],
		    queryconf['query'].bind(null, job.j_id,
		        job.j_phases.length - 1), worker.w_bus_options,
			worker.onRecord.bind(worker));
	});
	...
	job.j_phases.forEach(function (phase, pi) {
		...
		phase.p_reducers.forEach(function (reducer, ri) {
			if (mod_jsprim.isEmpty(worker.w_agents)) {
				worker.jobError(job, pi, 'EJ_NORESOURCES',
			...
			value['serverGeneration'] = worker.w_agents[
			    value['server']].a_record['value']['generation'];
			...
			if (pi > 0)
				worker.reducerInitKeyFile(reducer, job);
		});
	});
};
...
Worker.prototype.jobSave = function (job)
{
	...
	this.w_bus.batch(records, options, function (err) {
		...
		if (err) {
			...
			job.j_log.error(err, 'failed to save job record');
...
Worker.prototype.jobDone = function (job)
{
	...
	if (job.j_nlocates > 0 || job.j_nauths > 0)
		return (false);
...
Worker.prototype.processQueues = function ()
{
	...
	mod_assert.ok(this.w_auths_out.length === 0 ||
	    this.w_pending_auths == this.w_max_pending_auths);
};
...
Worker.prototype.taskCleanup = function (job, record)
{
	var worker = this;
	var phase, reducer, stream;
	var pi, ri;
	 ...
	 */
	if (record['value']['key']) {
		if (keyIsAnonymous(record['value']['key'], job.j_id))
	...
	job.j_log.info('taskCleanup: deleting keys for reduce task "%s"',
	    record['key']);

	pi = record['value']['phaseNum'];
	ri = record['value']['rIdx'];

	if (pi >= job.j_phases.length ||
	    job.j_phases[pi].p_type != 'reduce' ||
	    ri >= job.j_phases[pi].p_reducers.length) {
		job.j_log.error('taskCleanup: invalid reduce task', record);
		return;
	}
	...
	stream.on('error', function (err) {
		job.j_log.error(err, 'failed to open "%s" for read',
...
Worker.prototype.dispResolveUser = function (dispatch)
{
	...
	this.w_redis.get('/login/' + login, function (err, data) {
		var record;
...
Worker.prototype.dispLocate = function (dispatch)
{
	...
	dispatch.d_objname_internal = pathSwapFirst(
	    dispatch.d_objname, dispatch.d_account);
	this.w_locates_out.push(dispatch);
	job.j_log.debug('locate "%s": enqueued new request',
	    dispatch.d_objname_internal);
};
...
Worker.prototype.dispLocateResponse = function (dispatches, err, locations)
{
	...
	for (i = 0; i < dispatches.length; i++) {
		...
		dispatch.d_locations = locations[iobjname].filter(
		    function (loc) {
			return (worker.w_agents.hasOwnProperty(loc['host']));
		    });
		this.w_locates_in.push(dispatch);
	}
...
{
	var worker = this;
	var which, server, zonename, task, value, origin_value, phase;
	...
	if (dispatch.d_objectid == '/dev/null' &&
		...
		which = mod_jsprim.randElt(dispatch.d_locations);
		server = which['host'];
	...
	value = task.t_value;
	value['key'] = dispatch.d_objname;
	value['p0key'] = dispatch.d_pi === 0 ? dispatch.d_objname :
	    dispatch.d_origin['value']['p0key'];
	value['account'] = dispatch.d_account;
	value['objectid'] = dispatch.d_objectid;
	value['server'] = server;
	value['serverGeneration'] = this.w_agents[server].
	    a_record['value']['generation'];
	value['zonename'] = zonename;

	value['prevRecordType'] =
	    dispatch.d_origin['bucket'] == this.w_buckets['jobinput'] ?
	    'jobinput' : 'taskoutput';
...
Worker.prototype.dispReduce = function (dispatch)
{
	var worker = this;
	var job, pi, uuid;
	var task, reducer, phase, value, origin_value;
	...
	if (reducer.r_task === undefined) {
		dispatch.d_barrier.done(dispatch.d_id);
		/*
		 * XXX needs to dispatch an error or at least set timePropagated
		 * on previous record.
		 */
		return;
	}
	...
	value = {
	    ...
	    'key': dispatch.d_objname,
	    'p0key': dispatch.d_p0objname,
	...
	if (pi > 0 && keyIsAnonymous(dispatch.d_objname, job.j_id)) {
		if (reducer.r_anon_bufs)
			reducer.r_anon_bufs.push(dispatch.d_objname);
		else
			reducer.r_anon_stream.write(dispatch.d_objname + '\n');
	}
};
...
Worker.prototype.jobPropagateEnd = function (job, now)
{
	...
	for (pi = 0; pi < job.j_phases.length; pi++) {
		...
		if (phase.p_nuncommitted > 0 || phase.p_nunpropagated > 0)
			/* Tasks still running. */
			break;
	}
...
Worker.prototype.reduceEndInput = function (job, i, now)
{
	...
	phase.p_reducers.forEach(function (reducer) {
		...
		worker.w_bus.putBatch([
			...
			worker.jobError(job, i, 'EJ_INTERNAL',
...
{
	var worker = this;
	var queries = [ wQueries.wqJobsAbandoned, wQueries.wqJobsCreated ];

	if (doquiesce && this.w_quiesced === undefined) {
		this.w_quiesced = Date.now();
		queries.forEach(function (queryconf) {
			worker.w_bus.unsubscribe(
			    worker.w_subscrips[queryconf['name']]);
		});

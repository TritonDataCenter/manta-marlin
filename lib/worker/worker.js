...
mwJobState.prototype.jobAssign = function ()
{
	...
	this.js_moray.assignJob(record, function (err) {
		...
		job.transition('unassigned', 'uninitialized');
	});
};
...
mwJobState.prototype.loadTaskGroups = function (groups, phasenum)
{
	 ...
	 * and completed keys.  That's because task group records may have
	...
	for (tgid in phase.p_groups) {
		...
		phasegroup['results'].forEach(function (result) {
			...
			phase.p_keys_done[result['input']] = true;
		});
	}
};
...
mwJobState.prototype.onTaskGroup = function (taskgroups)
{
	 ...
	 */
	this.js_log.info('received taskgroup update', taskgroups);
...
function main()
{
	...
	moray = new mod_moray.MockMoray({
	    ...
	    'taskGroupInterval': mwConf['taskGroupInterval'],

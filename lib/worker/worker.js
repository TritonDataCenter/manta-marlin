...
Worker.prototype.jobUpdate = function (record)
{
	...
	if (job.j_input_done === undefined &&
		...
		if (i < job.j_phases.length && phase.p_reducers) {
			phase.p_reducers.forEach(function (reduce) {
				var task = reduce.r_task;
				...
				task.t_value['timeInputDone'] =
				    mod_jsprim.iso8601(Date.now());
				task.t_value['nInputs'] = reduce.r_ninput;

				var options = {};
				if (task.t_record)
					options['etag'] =
					    task.t_record['_etag'];
				worker.w_queue.dirty('task', task.t_id,
				    task.t_value, options);
			});
		}
...
{
	var job, pi, ri;
	var task, reducer, record, phase, i, opts;
	...
	process.nextTick(function () {
		for (i = 0; i < phase.p_reducers.length; i++) {
			...
			if (!reducer.r_task ||
			    reducer.r_task.t_value['timeInputDone'])
				continue;
			...
			job.j_log.debug('dirtying task "%s" ' +
			    '(input now complete)', task.t_id);
			...
			if (task.t_record)
				...
				opts = {};
			worker.w_queue.dirty('task', task.t_id, task.t_value,
			    opts);
		}
	});
};

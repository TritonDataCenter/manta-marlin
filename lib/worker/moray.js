...
RemoteMoray.prototype.findObjects = function (bucket, filter, options)
{
	...
	this.initClient(function () {
		moray.rm_npending++;
		var req = moray.rm_client.findObjects(bucket, filter, options);
...
{
	/*
	 * XXX As an optimization, we could use last_mtime to avoid querying for
	 * our own jobs which haven't changed since our last poll -- except on
	 * initial startup, when we do want to include such jobs.
	 */
	var query = mod_extsprintf.sprintf(
	     '&(!(timeDone=*))(|(!(worker=*))(worker=%s)(_mtime<=%d))',
	     worker_uuid, Date.now() - abandon_time);

	return (query);
};

RemoteMoray.prototype.filterJobIds = function (jobids, last_id)
{
	if (jobids.length === 0)
		return (null);

	return (mod_extsprintf.sprintf('&(|%s)(_id>=%d)', jobids.map(
	    function (j) { return ('(jobId=' + j + ')'); }), last_id + 1));
};
...
RemoteMoray.prototype.filterTasks = function (jobids, last_mtime)
{
	if (jobids.length === 0)
		return (null);

	return (mod_extsprintf.sprintf('&(|%s)(_mtime>=%d)', jobids.map(
	    function (j) { return ('(jobId=' + j + ')'); }), last_mtime));
};

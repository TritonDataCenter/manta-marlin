...
function MockMoray(args)
{
	...
	this.mm_tg = new mod_mrutil.Throttler(
 ...
 * they're stored on.  This implementation is for testing only and distributes
 * keys randomly to three nodes using a fake zonename.
 */
...
RemoteMoray.prototype.setup = function (callback)
{
	...
	this.rm_setup = RemoteMoraySetup({
	    'moray': this.rm_client,
	    'jobsBucket': this.rm_bkt_jobs,
	    'taskGroupsBucket': this.rm_bkt_taskgroups
	}, function (err) {
...
/*
 * XXX This implementation is stubbed out for testing Marlin in a mock
 * environment.  Unlike the version in MockMoray (which is used for mock
 * testing), this implementation is intended to work with a real agent
 * configured in a development environment, where the agent lives on a headnode
 * and the mock manta instance is in the same zone as this worker.
 */
RemoteMoray.prototype.mantaLocate = function (keys, callback)
{
	...
	/*
	 * This isn't pretty, but it's a reliable, synchronous way to extract
	 * our own zonename.
	 */
...
RemoteMoray.prototype.mantaLocateTest = MockMoray.prototype.mantaLocate;
...
function RemoteMoraySetup(args, callback)
{
	...
	return (mod_vasync.forEachParallel({
		...
		moray.putBucket(bucket, { 'schema': schemas[bucket] },
		    subcallback);
	    }
	}, callback));
}

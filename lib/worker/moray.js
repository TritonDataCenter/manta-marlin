...
MockMoray.prototype.restify = function (server)
{
	...
	server.get('/:bucket/:key', function (request, response, next) {
		var rv = moray.get(
	...
	server.get('/:bucket', function (request, response, next) {
		var bucket, rv;
	...
	server.put('/:bucket/:key', function (request, response, next) {

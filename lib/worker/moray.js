...
function RemoteMoray(args)
{
	...
	this.rm_schemas = mod_schema.sBktSchemas;
...
RemoteMoray.prototype.setup = function (callback)
{
	...
	this.rm_setup = RemoteMoraySetup({
	    ...
	    'bktJobs': this.rm_bkt_jobs,
	    'bktTaskGroups': this.rm_bkt_taskgroups,
	    'schemas': this.rm_schemas
	}, function (err) {
...
RemoteMoray.prototype.mantaLocateTest = function (keys, callback)
{
	var rv = {};
	var i = 0;
...
function RemoteMoraySetup(args, callback)
{
	...
	mod_assert.ok(!args.hasOwnProperty('bktJobs') ||
	    typeof (args['bktJobs']) == 'string');
	mod_assert.ok(!args.hasOwnProperty('bktTaskGroups') ||
	    typeof (args['bktTaskGroups']) == 'string');
	mod_assert.ok(!args.hasOwnProperty('schemas') ||
	    typeof (args['schemas']) == 'object');

	var moray = args['moray'];
	var bktJobs = args['bktJobs'] || mod_config.mcBktJobs;
	var bktTgs = args['bktTaskGroups'] || mod_config.mcBktTaskGroups;
	var schemas = args['schemas'] || mod_schema.sBktSchemas;

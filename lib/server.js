...
var mod_worker = require('./worker/worker');

var mlUsage = [
    'Usage: node server.js [-l port] configfile',
    '',
...
function main()
{
	...
	conf = mod_mautil.readConf(mlLog, mod_worker.mwConfSchema,
	    process.argv[parser.optind()]);

	mlBktJobs = conf['jobsBucket'];
	mlBktTaskGroups = conf['taskGroupsBucket'];
	...
	conf['moray'] = mlClient;
	mod_marlin_moray.RemoteMoraySetup(conf, function (err) {
		if (err) {
...
function jobSubmit(request, response, next)
{
	...
	input['outputKeys'] = [];
	input['discardedKeys'] = [];
	input['phaseResults'] = input['phases'].map(
	    function () { return ([]); });

...
function jobFetchErrorsImpl(api, jobid, options, extra)
{
	...
	req.on('record', function (record) {
		...
		var errobj = {
		    ...
		    'server': value['server'],
		    'mantaComputeId': value['mantaComputeId'],
...
/*
 * jobArchiveDone(job, options, callback): wrasse uses this to mark a job as
 * starting archival (and locks).  Updates the `timeArchiveStarted` and
 * `wrasse` fields.
 *
...
/*
 * jobArchiveDone(job, options, callback): wrasse uses this to mark a job as
 * completely archived.  Only updates the `timeArchiveDone` field.
 *
 * Upon completion, invokes callback(err, record), where "record" is the
 * *previous* job record (which can be useful for warning on odd conditions.
 */
...
function taskDoneWriteOthers(arg, callback)
{
	...
	if (arg.nout === undefined) {
		...
		client.putObject(bucket, uuid, {
		    ...
		    'taskId': task['taskId'],
		    'server': task['server'],
		    'mantaComputeId': task['mantaComputeId'],

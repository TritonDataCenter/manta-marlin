...
function jobFetchErrors(api, jobid, options)
{
	...
	req.on('record', function (record) {
		...
		var errobj = {
		    ...
		    'p0key': value['p0input'],
		    'what': summary,
		    'code': value['errorCode'] || 'EJ_UNKNOWN',
...
function jobFetchFailedJobInputs(api, jobid, options)
{
	...
	req.on('err', function (error) {
		if (!error['p0key'])
			return;

		rv.emit('key', error['p0key']);
	});
...
function taskDoneWriteOthers(arg, callback)
{
	...
	if (arg.nout === undefined) {
		...
		client.putObject(bucket, uuid, {
		    ...
		    'input': task['key'] || undefined,
		    'p0input': task['p0key'] || undefined,
		    'taskId': task['taskId'],
		    'server': task['server'],

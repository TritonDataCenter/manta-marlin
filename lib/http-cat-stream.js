...
function HttpCatStream(options)
{
	 ...
	 * we're processing.  They are divided into three sets:
	 *
	 * 1. Fields are set when we attempt to fetch an object for the first
	 *    time:
	 *    hcs_current		current resource, as passed to write()
	 *    hcs_bytes_read		the total number of bytes that have been
	 *				read over all retries for the object
	 *    hcs_resume_count		inited to 0, the number of times a
	 *				resume has been attempted
	 *    hcs_md5			the cryptoHash object kept up to date
	 *				with the bytes emitted to the output
	 *				stream
	 *
	 * 2. Fields that are set when we get the first response for an object:
	 *    hcs_exp_md5		the md5 checksum for the requested
	 *				object
	 *    hcs_exp_len		the length in bytes for the requested
	 *				object
	 *    hcs_etag			the etag of the requested object
	 *
	 * 3. Fields that are set for each request/resume:
	 ...
	 *    allocating socket		hcs_current, hcs_bytes_read,
	 *				hcs_resume_count, hcs_md5 assigned,
	 *				others undefined
	 *    request sent		hcs_current, hcs_bytes_read,
	 *				hcs_resume_count, hcs_md5 assigned,
	 *				hcs_request assigned, others undefined
	...
	client.get(getOpts, function (err, request) {
		...
		request.on('result', function (err2, response) {
			...
			response.on('end', function () {
				...
				/*
				 * As of this writing, there is a bug in node
				 * where no error is emitted when the server
				 * sends a socket RST.  When this occurs node
				 * acts like the request was successful.
				 * Fortunately we can detect this case by the
				 * the number of bytes expected vs the number
				 * of bytes read and resume the request.
				 */

...
AsyncDnsCache.prototype.add = function (hostname)
{
	...
	this.dns_log.debug('adding hostname "%s"', hostname);
};
...
AsyncDnsCache.prototype.kickEntry = function (hostname)
{
	...
	r.on('message', function (err, answer) {
		...
		cache.dns_log.debug('answers for hostname "%s" from "%s"',
	...
	this.dns_log.debug('attempting to resolve "%s" from "%s"', hostname, s);
 ...
 * such address exists.
 */
AsyncDnsCache.prototype.resolve4 = function (hostname)
{

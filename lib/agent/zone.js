 ...
 * a few seconds.
 *
 * When an individual task completes, fails, or times out, we may choose to
 * dispatch more tasks as part of the same job.  If not, or when the entire job
 * has completed, we mark the state "uninitialized" again (since the internal
 * state is arbitrarily changed by the user's task) and then transition it to
 * "ready" so that it can run another job's tasks.
 *
 *
 * Crash Recovery
 *
 ...
 * this task may complete successfully.  In this case, more than one agent may
 * successfully complete a task, in which case the job workers will pick which
 * one's output to use.
 */
...
var kstatClasses = [ 'zone_memory_cap', 'zone_zfs', 'zone_vfs' ];
...
maZone.prototype.stats = function ()
{
	mod_assert.ok(this.z_zoneid !== undefined,
	    'zoneid not known (stats() invoked after zone not booted?)');

	var rv = {};
	var k, stats, err;
	for (k in this.z_kstats) {
		stats = this.z_kstats[k].read();
		mod_assert.ok(Array.isArray(stats));
		if (stats.length != 1) {
			err = new VError('failed to read kstat "%s" for ' +
			    'zoneid %d (zonename "%s"): got %d instead of 1',
			    k, this.z_zoneid, this.z_zonename, stats.length);
			this.z_log.error(err);
			return (err);
		}

		if (stats[0]['data']['zonename'] != this.z_zonename) {
			err = new VError('failed to read kstat "%s" for ' +
			    'zoneid %d (zonename "%s"): got stat for zone ' +
			    '"%s" instead', k, this.z_zoneid, this.z_zonename,
...
function maZoneExec(zone, cmd, args, callback)
{
	...
	zone.z_log.debug('invoking "%s"', cmdstr);
	...
	mod_child.execFile(cmd, args, {}, function (err, stdout, stderr) {
		...
		if (err) {
			err = new VError(err, 'command "%s" ' +
			    'failed with stderr: %s', cmdstr, stderr);
			zone.z_log.debug(err);
			callback(err);
			return;
		}

		zone.z_log.debug('command "%s" ok', cmdstr);
...
{
	if (zone.z_kstats !== undefined) {
		for (var k in zone.z_kstats)
			zone.z_kstats[k].close();
	}

	zone.z_kstats = undefined;
...
function maZoneReadyKstat(zone, callback)
{
	...
	zone.z_kstats = {};
	kstatClasses.forEach(function (klass) {
		zone.z_kstats[klass] = new mod_kstat.Reader({
		    'class': klass,
		    'instance': zone.z_zoneid
		});
	});
...
{
	var args = [ 'update', zone.z_zonename ];
	var used = [];
	var key;
	...
	zone.z_log.info('setting zone properties', options);
	zone.z_options_pending = true;
	maZoneExec(zone, 'vmadm', args, function (err) {
		...
		used.forEach(function (ukey) {
			zone.z_options[ukey] = options[ukey];
		});

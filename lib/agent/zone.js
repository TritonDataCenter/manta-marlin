 ...
 * dispatch more tasks as part of the same task group.  If not, or when the
 * entire task group has completed, we mark the state "uninitialized" again
 * (since the internal state is arbitrarily changed by the user's task) and then
 * transition it to "ready" so that it can run another task group's tasks.
 *
 *
 * Crash Recovery
 *
...
maZone.prototype.httpState = function ()
{
	...
	if (this.z_taskgroup)
		obj['taskGroupId'] = this.z_taskgroup.t_id;
	if (this.z_sockfd)
...
/*
 * The first two stages clean up server and socket state left over from a
 * previous run.  In the common (success) case, the previous run created a zsock
 * fd and created a server that's listening on it.  We must close the server so
 * that libuv knows to stop calling accept(2) on it, and this operation also
 * closes the underlying socket fd.  In the case where the last run failed to
 * set up the server after having successfully created the socket, it should
 * have cleaned up the socket.  So this cleanup stage can always assume that
 * either the server has been initialized, or none of the server/socket fields
 * are initialized.
 */

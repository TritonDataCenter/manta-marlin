...
maZoneManager.prototype.zoneMakeReady = function (zone)
{
	...
	zone.z_pipeline = mod_asyncutil.asPipeline({
	    'arg': zone,
	...
	maZoneExec(zone, 'svcs', [ '-H', '-z', zone.z_zonename, '-ostate',
		...
		if (!err) {
			...
			if (state == 'maintenance') {
				callback(new mod_jsutil.jsError(null,
...
function maZoneReadyZsock(zone, callback)
{
	...
	mod_zsock.createZoneSocket({
		...
		zone.z_server = mod_restify.createServer({
		    'name': zone.z_zonename + ' agent server',
		    'log': zone.z_log
		});

		/*
		 * We can skip the accept and queryParser, since we know exactly
		 * what kinds of requests the client will be making.
		 */
		zone.z_server.use(mod_restify.bodyParser({
		    'mapParams': false
		}));

		zone.z_server.get('/params', function (request, response,
		    next) {
			/* XXX */
			response.send({ 'test params': 'okay' });
			next();
		});

		zone.z_server.on('after', mod_restify.auditLogger({
		    'log': zone.z_log
		}));

		zone.z_server.on('error', function (suberr) {
		...
		zone.z_server.server.listenFD(zone.z_sockfd, function () {
...
function maZoneReadyDropAgent(zone, callback)
{
	...
	mod_fs.mkdir(dst, function (err) {
		if (err) {
			callback(new mod_jsutil.jsError(err, '"mkdir" failed'));
			return;
		}

		mod_fsutil.mountLofs(src, dst, callback);
	});
}

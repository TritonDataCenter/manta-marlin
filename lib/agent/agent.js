...
function maTask(args)
{
	...
	this.t_job_phases = mod_apiutil.mlDeepCopy(args['job']['phases']);
...
function maHttpZonesList(request, response, next)
{
	...
	for (var zonename in maZoneManager.mz_zones) {
		var zone = maZoneManager.mz_zones[zonename];
		var obj = {
			'zonename': zone.z_zonename,
			'state': zone.z_state
		};

		if (zone.z_task)
			obj['taskId'] = zone.z_task.t_id;
		if (zone.z_sockfd)
			obj['sockfd'] = zone.z_sockfd;
		if (zone.z_pipeline)
			obj['pipeline'] = zone.z_pipeline;
		if (zone.z_timeout)
			obj['timeout'] = true;
		if (zone.z_pending_command)
			obj['pendingCommand'] = zone.z_pending_command;

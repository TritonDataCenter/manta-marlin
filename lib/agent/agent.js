...
function pfJob(last_id)
{
	if (mod_jsprim.isEmpty(maJobs))
		return (null);

	return (mod_extsprintf.sprintf('&(_id>=%d)(|%s)',
	    last_id + 1, Object.keys(maJobs).map(
	    function (jobid) { return ('(jobId=' + jobid + ')'); }).join('')));
}

function pfTask(last_id)
{
	var rv = '&(_id>=' + (last_id + 1) + ')' +
	    '(server=' + maServerName +')' +
	    '(!(state=done))' +
	    '(!(state=aborted))';
...
function pfTaskInput(last_id)
{
	...
	mod_jsprim.forEachKey(maTasksReduce, function (taskid) {
		...
		if (task.t_input_done && maMorayLastPolled['taskinput'] &&
		    ...
		    (task.t_started || task.t_xinput.length > 0))
...
function maJob(jobid, interval)
{
	...
	this.j_save_interval = interval;	/* save frequency */
...
function maTask(record)
{
	...
	this.t_started = false;			/* task started running */
}

...
mAgent.prototype.schedHogsTick = function (timestamp)
{
	...
	mod_jsprim.forEachKey(this.ma_taskgroups, function (_, group) {
		if (group.g_hog === undefined ||
		    timestamp - group.g_hog < grace)
			return;

		if (group.g_hog_last !== undefined &&
		    timestamp - group.g_hog < steal)
			return;
...
mAgent.prototype.schedHogKill = function (group, timestamp)
{
	...
	/*
	 * We pick the stream that started the latest, in hopes that killing
	 * that one will do the least damage.  On the other hand, if we find one
	 * that's still initializing, we'll take that one.
	 */
	mod_jsprim.forEachKey(group.g_streams, function (sid, astream) {
		if (astream.s_task === undefined) {
			stream = astream;
			latest = Infinity;
		} else if (latest === undefined || astream.s_start > latest) {
			stream = astream;
	...
	if (stream.s_task !== undefined) {
		...
		this.ma_dtrace.fire('sched-task-killed', function () {
...
function maTaskApiManta(request, response, next)
{
	...
	var iostream, proxyargs, reducer, donefunc;

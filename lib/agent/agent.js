...
function maTaskApiSetup(zone, s)
{
	 ...
	 */
	function maTaskApiInit(request, response, next) {
		request.maZone = zone;
		next();
	}
	 ...
	 * 204 instead.
	 */
	function setTaskTimer(request, response, next) {
		request.connection.setTimeout(2 * maRequestTimeout);

		request.maWaiter = new maZoneWaiter(request,
						    response,
						    next);
	...
	function clearTimer(request, response, next) {
	...
	function writeContinue(request, response, next) {
	...
	s.get('/my/jobs/task/task', writeContinue, maTaskApiInit, setTaskTimer,
	    clearTimer);
	s.post('/my/jobs/task/commit',
	    writeContinue,
	    mod_restify.bodyParser({ 'mapParams': false }),
	    maTaskApiInit, maTaskApiCommit);
	s.post('/my/jobs/task/fail',
	    writeContinue,
	    mod_restify.bodyParser({ 'mapParams': false }),
	    maTaskApiInit, maTaskApiFail);
	s.post('/my/jobs/task/live',
	    writeContinue, maTaskApiInit, maTaskApiLive);
	...
	methods.forEach(function (method) {
		/* JSSTYLED */
		s[method](/.*/, maTaskApiInit, maTaskApiManta);
	});
}
 ...
 */
function maTaskApiTask(request, response, cb, nowait)
{
	var agent = maAgent;
	var zone = request.maZone;
	var state;

...
function mAgent(filename)
{
	...
	this.ma_kstat_fields = {};
	maKstatCumulativeFields.forEach(
	    function (f) { agent.ma_kstat_fields[f] = true; });
...
mAgent.prototype.initZones = function ()
{
	...
	var lookupZones = function (tag, callback) {
		var cmd = sprintf('vmadm lookup tags.manta_role=%s', tag);

		agent.ma_log.info('listing zones with "%s"', cmd);

		mod_child.exec(cmd, function (err, stdout, stderr) {
			if (err) {
				agent.ma_log.fatal(err,
				    'failed to read list of zones: %s',
				    stderr);
				throw (err);
			}
	...
	/*
	 * Local zones are identified with the 'compute' tag.  Before MANTA-1157
	 * was integrated, those zones were identified with 'marlincompute'.
	 * Look for zones with both tags to avoid a flag day.
	 */
	var tags = [ 'compute', 'marlincompute' ];
...
mAgent.prototype.zonesCheckLiveness = function ()
{
	...
	mod_jsprim.forEachKey(this.ma_zones, function (zonename, zone) {
		...
		if (stream.s_group.g_multikey &&
		    !agent.taskStreamHasWork(stream))
			return;

		timestamp = Date.now();
		if (agent.streamSawAllocFailures(stream)) {
			/*
			 * One possible source of lackey timeouts is if the
			 * lackey ran out of memory.  We can detect this by
			 * looking for anonymous allocation failures in the
			 * zone.  If we find any, we'll report a more specific
			 * error and not bother taking the zone out of service.
			 */
			...
			/*
			 * A lackey timeout is a pretty serious failure.  We
			 * disable the zone.  Eventually we'll need alarms on
			 * this to make it clear that operator intervention is
			 * required to diagnose and clear the failure.  We could
			 * potentially even skip this by collecting core files
			 * dumped by the zone, logs from the zone, a ptree of
			 * the zone, and a core of any running lackey process,
			 * and then resetting the zone.
			 */
			zone.z_log.error('lackey has timed out');
...
/*
 * Returns the cumulative total of monitored kstats for the stream's zone, minus
 * the values we recorded when the stream was dispatched.
 */
mAgent.prototype.streamStatsDiff = function (stream)
{
	var stats, diff, hrtimestamp;
	var k, d, o, n, f;

	mod_assert.ok(stream.s_machine !== undefined);
	mod_assert.ok(stream.s_initial_stats !== undefined);

	stats = this.streamStats(stream);
	if (stats instanceof Error)
		return (stats);

	hrtimestamp = process.hrtime(stream.s_initial_stats['hrtimestamp']);

	diff = {};
	diff['hrtimestamp'] = hrtimestamp[0] * NANOSEC + hrtimestamp[1];
	...
	for (k in stats) {
		 ...
		 */
		mod_assert.equal(o['class'], n['class']);
		mod_assert.equal(o['module'], n['module']);
		mod_assert.equal(o['name'], n['name']);
		mod_assert.equal(o['instance'], n['instance']);
		mod_assert.equal(o['data']['zonename'], n['data']['zonename']);

		d['snaptime'] = n['snaptime'] - o['snaptime'];
		mod_assert.ok(d['snaptime'] >= 0);
...
{
	var stats = this.streamStatsDiff(stream);
...
mAgent.prototype.taskCommit = function (stream, body, now, callback)
{
	 ...
	 * necessarily done, as are reduce tasks that have processed all inputs.
	 * But reduce tasks that haven't finished reading all inputs will be
	 * left in the zone until we finish reading them all.  This greatly
	 * simplifies the processing of this presumably uncommon case, since
	 * otherwise we'd need to invalidate newly written taskinput records.
	 */
	if (!group.g_multikey ||
	    (this.taskReadAllRecords(task) && task.t_xinput.length === 0)) {
...
function maTaskStreamDispatch(arg, callback)
{
	...
	stats = agent.streamStats(stream);
	if (!(stats instanceof Error) &&
	    stats['memory']['data']['anon_alloc_fail'] > 0)
		stats = new VError(
...
function maTaskApiSetup(zone, s)
{
	...
	s.get('/my/jobs/task/task', writeContinue, init, setTimer, clearTimer);
	...
	s.post('/my/jobs/task/live', writeContinue, init, maTaskApiLive);
...
function maTaskApiManta(request, response, next)
{
	...
	if (request.method == 'PUT' && !mod_mautil.isMantaDirectory(type)) {
		/*
	...
	if (iostream === 'stdout' && group.g_intermediate)
		proxyargs['server']['headers']['x-durability-level'] = 1;
...
function maTaskStream(stream_id, group, machine)
{
	...
	this.s_initial_stats = undefined;	/* initial zone stats */

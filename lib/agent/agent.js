...
function maTask(args)
{
	...
	this.t_outputkeys = args['taskOutputKeys'];
...
maTask.prototype.httpState = function ()
{
	var rv = {
	    ...
	    'taskOutputKeys': this.t_outputkeys,
	    'taskPartialKeys': this.t_partial,
	    'taskDiscardedKeys': this.t_discardedkeys
	};
...
/*
 * Checks whether the task associated with the given zone can accept Task
 ...
 */
function maTaskMergePartialKeys(task, success)
{
	var seen, list;

	if (success) {
		list = task.t_outputkeys;
		seen = task.t_seenoutput;
	} else {
		list = task.t_discardedkeys;
		seen = task.t_seendiscarded;
	}

	task.t_partial.forEach(function (key) {
...
function maTaskApiCommit(request, response, next)
{
	...
	mod_assert.equal(task.t_state, maTask.TASK_S_RUNNING);
	...
	if (task.t_seenfail.hasOwnProperty(key)) {
		next(new mod_restify.ConflictError(
		    'key "' + key + '" has already failed'));
		return;
	}

	if (!task.t_seendone.hasOwnProperty(key)) {
		maTaskMergePartialKeys(task, true);
		task.t_seendone[key] = true;
...
function maTaskApiFail(request, response, next)
{
	...
	if (!task.t_seenfail.hasOwnProperty(key)) {
		maTaskMergePartialKeys(task, false);
		task.t_seenfail[key] = true;
...
function maTaskApiManta(request, response, next)
{
	...
	request.maWroteHeader = false;

	if (!task.t_seenpartial.hasOwnProperty(path)) {
		task.t_seenpartial[path] = true;
	...
	var onerror = function (err) {
		if (request.maError || request.maWroteHeader)
			return;

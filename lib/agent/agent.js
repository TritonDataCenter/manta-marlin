...
function maTask(record)
{
	...
	this.t_ninputs = undefined;		/* number of inputs */
...
function maTaskUpdated(task, record)
{
	...
	if (record['value']['state'] == 'cancelled') {
		...
		if (group.g_task == task) {
			...
			group.g_failmsg = 'pending task cancelled externally';
			maTaskGroupAbortQueued(group, {
			    'code': 'EJ_INTERNAL',
			    'message': 'internal error',
			    'messageInternal': group.g_failmsg
			});
...
function maTaskGroupAbortQueued(group, error)
{
	var now = Date.now();
...
function maTaskGroupAbort(group)
{
	...
	group.g_rqqueue.push(function (callback) {
		...
		if (group.g_task && !group.g_task.t_record['value']['result']) {
			maTaskError(group.g_task, Date.now(), {
			    'code': 'EJ_INTERNAL',
...
maTaskGroup.prototype.httpState = function ()
{
	...
	if (task) {
		...
		rv['task_nxinput'] = task.t_xinput.length;
	}
...
maTaskGroup.prototype.taskState = function ()
{
	...
	if (this.g_multikey) {
		 ...
		 * we've read the proper number of taskinput records.
		 */
...
function maTaskGroupsPoke()
{
	...
	while (maZonesReady.length > 0 && maTaskGroupsQueued.length > 0) {
		...
		maLog.info('assigning group "%s" to zone "%s"',
	...
	zone.z_hyprlofs.removeAll(function (err) {
		...
		if (err) {
			...
			maTaskGroupAbortQueued(group, {
			    'code': 'EJ_INTERNAL',
			    'message': 'internal error',
			    'messageInternal': 'failed to unmap hyprlofs'
			});
			maTaskGroupAdvance(group, callback);
			return;
		}
...
function maTaskApiSetup(zone, s)
{
	...
	s.on('expectContinue', function (request, response, route) {
		 ...
		 * have the upstream's "100 Continue".
		 */
...
function maTaskApiFail(request, response, next)
{
	...
	group.g_rqqueue.push(function (callback) {
		...
		maTaskError(group.g_task, now, error);
		maTaskGroupAdvance(group, callback);
	}, function (sent) {
		if (!sent)
			response.send(204);
...
{
	var zone, key, group, proxyargs, reducer, stream, continuefunc;
	...
	if (request.method == 'PUT' &&
		...
		if (request.headers['x-marlin-reference']) {
			maTaskEmit(group, group.g_task, stream, key, reducer);
			response.send(204);
			next();
			return;
		}
...
function maTaskEmit(group, task, stream, key, reducer)
{
	if (group.g_task != task) {
		group.g_log.error('group has moved on by the time %s key ' +

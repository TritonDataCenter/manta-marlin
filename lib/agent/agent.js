...
var maLogStreams = [ { 'stream': process.stdout, 'level': 'debug' } ];
var maZoneAutoReset = true;	/* false for debugging only */
var maZoneLivenessInterval = 10000;
var maHost, maPort, maMantaHost, maMantaPort;
var maTaskGroupInterval, maTaskGroupSaveInterval;
var maTaskGroupIdleInterval, maTickInterval;
...
var maConfSchema = {
	...
	'taskGroupIdleInterval': mod_schema.sIntervalRequired,
	'taskGroupSaveInterval': mod_schema.sIntervalRequired,
	'tickInterval': mod_schema.sIntervalRequired,
...
function usage(errmsg)
{
	...
	console.error('usage: node agent.js [-o logfile] [-p port] conffile');
	process.exit(2);
}
...
function main()
{
	...
	var conf = mod_mautil.readConf(maLog, maConfSchema,
	    process.argv[parser.optind()]);

	maLog.info('configuration', conf);

	maHost = conf['instanceUuid'];
	maPort = conf['port'];
	maMantaHost = conf['manta']['host'];
	maMantaPort = conf['manta']['port'];
	maTaskGroupInterval = conf['taskGroupInterval'];
	maTaskGroupIdleInterval = conf['taskGroupIdleInterval'];
	maTaskGroupSaveInterval = conf['taskGroupSaveInterval'];
	maTickInterval = conf['tickInterval'];
	...
	maDnsCache = new mod_adnscache.AsyncDnsCache({
	    ...
	    'nameServers': conf['dns']['nameservers'].slice(0),
	    'triggerInterval': conf['dns']['triggerInterval'],
	    'graceInterval': conf['dns']['graceInterval'],
	...
	maMoray = new mod_moray.RemoteMoray({
	    'dns': maDnsCache,
	    'host': conf['moray']['host'],
	    'port': conf['moray']['port'],
	    'log': maLog.child({ 'component': 'remote-moray' }),
...
function maSaveTaskGroups()
{
	...
	mod_jsprim.forEachKey(maTaskGroups, function (tgid, group) {
		...
		if (group.t_save_pending_time === undefined ||
		    now - group.t_save_pending_time > maTaskGroupIdleInterval)
			group.t_dirty++;
		...
		group.t_save_pending_time = Date.now();
	 ...
	 */
	maLog.debug('starting task group save');
...
function maCheckZonesLiveness()
{
	...
	mod_jsprim.forEachKey(maZones, function (zonename, zone) {
		...
		if (now - zone.z_last_contact > maZoneLivenessInterval) {
			...
			taskgroup.t_log.warn('aborting (internal timeout)');
			taskgroup.t_failmsg = 'internal timeout';
			maTaskGroupAbort(taskgroup);
		}
	});
}

function maTaskGroupAbort(taskgroup)
{
	taskgroup.t_rqqueue.push(function (callback) {
		if (taskgroup.t_state != maTaskGroup.TASKGROUP_S_RUNNING)
			return;
...
function maTaskGroup(args)
{
	...
	this.t_phase = mod_jsprim.deepCopy(args['phase']);
	...
	this.t_map_keys = this.t_phase['type'] == 'storage-map';
	this.t_multikey = this.t_phase['type'] == 'reduce';
	 ...
	 * represents the execution of the user's code on some of the input
	 * keys.  For generic and storage-map phases, there's always one input
	 * key.  Reduce phases may have many input keys.  Task execution may
	 * emit any number of output keys, which are recorded in
	 * t_current_outputs (an array, to preserve order) and t_current_seen
	 * (an object, to efficiently ignore duplicate keys).
	 */
	this.t_current_start = undefined;	/* start time for this key */
	this.t_current_inputs = undefined;	/* input keys */
	...
	this.t_save_pending_time = undefined;
}
...
maTaskGroup.prototype.httpState = function ()
{
	...
	if (this.t_current_inputs)
		rv['currentInputs'] = this.t_current_inputs;
...
maTaskGroup.prototype.taskState = function ()
{
	...
	mod_assert.ok(this.t_current_inputs !== undefined);

	rv = {
	    'taskPhase': this.t_phase,
	    'taskGroupId': this.t_id,
	    'taskInputKeys': this.t_current_inputs,
	    'taskOutputKeys': this.t_current_output
	};

	if (this.t_phase['type'] == 'reduce') {
		/*
	...
	if (this.t_map_keys) {
		mod_assert.equal(1, this.t_current_inputs.length);
		rv['taskInputFile'] = mod_path.join(
		    maZones[this.t_machine].z_manta_root,
		    this.t_current_inputs[0]);
	}
...
function maTaskGroupAdvance(taskgroup, callback)
{
	...
	if (taskgroup.t_cancelled || taskgroup.t_failmsg ||
		...
		taskgroup.t_current_inputs = undefined;
	...
	if (taskgroup.t_multikey) {
		taskgroup.t_current_start = now;
		taskgroup.t_current_inputs = taskgroup.t_input.map(
	...
	if (!taskgroup.t_map_keys) {
		taskgroup.t_current_start = now;
		taskgroup.t_current_inputs = [ key['key'] ];
		taskgroup.t_current_outputs = [];
		taskgroup.t_current_seen = {};
		callback();
		return;
	...
	if (key['key'][0] != '/') {
		taskgroup.t_log.error('failed to load "%s": ' +
		    'does not start with "/"', key['key']);
		taskgroup.t_dirty++;
		taskgroup.t_results.push({
			...
			'error': {
				'code': 'EJ_NOENT',
				'message': 'failed to load "' + key['key'] + '"'
			}
		});
	...
	zone.z_hyprlofs.removeAll(function (err) {
		...
		var account = key['key'].split('/')[1];
		var rootkeypath = mod_path.join('/zones', key['zonename'],
		    'root', 'manta', account, key['objectid']);
...
function maTaskGroupCancel(taskgroup)
{
	...
	taskgroup.t_cancelled = new Date();
	taskgroup.t_log.info('taskgroup cancelled');
	maTaskGroupAbort(taskgroup);
}
...
function maTaskGroupMarkDone(taskgroup)
{
	...
	if (!zone.z_failed && maZoneAutoReset) {
		...
		if (zone.z_failed)
			zone.z_log.warn(
			    'disabling zone due to previous failure');
	}
}
...
function maTaskApiCommit(request, response, next)
{
	...
	if (!taskgroup.t_multikey && !body.hasOwnProperty('key')) {
	...
	taskgroup.t_rqqueue.push(function (callback) {
		 ...
		 */
		if (!taskgroup.t_multikey) {
			mod_assert.equal(taskgroup.t_current_inputs.length, 1);
			var key = body['key'];

			if (key !== taskgroup.t_current_inputs[0]) {
				next(new mod_restify.ConflictError('key "' +
				    key + '" is not the current key ("' +
				    taskgroup.t_current_inputs[0] + '")'));
				callback();
				return;
			}
		}
		...
		taskgroup.t_dirty++;
		taskgroup.t_current_inputs.forEach(function (p, i) {
			var out = i === 0 ?
			    taskgroup.t_current_outputs : [];
			taskgroup.t_results.push({
			    'input': p,

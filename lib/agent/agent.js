...
function maMorayOnRecord(record)
{
	...
	if (maTasksDeathrow.hasOwnProperty(taskid) &&
	    maMorayQueue.pending('task', taskid)) {
...
function maCheckTaskCount()
{
	...
	mod_jsprim.forEachKey(maTaskGroups, function (_, group) {
		...
		mod_jsprim.forEachKey(group.g_streams, function (__, stream) {
			 ...
			 */
			if (stream.s_task &&
			    stream.s_task.t_record['value']['result'] ===
			    undefined &&
			    !maJobs[group.g_jobid].j_cancelled)
				rawcount++;
		});
	});
	...
	if (rawcount != maNTasks || rawcount != taskcount) {
		maLog.fatal('maCheckTaskCount failed: maNTasks = %d, ' +
		    'size of maTasks = %d, sum of tasks found = %d',
		    maNTasks, taskcount, rawcount);
		process.abort();
	}
}
...
function maTaskStreamState(stream)
{
	 ...
	 *
	 * The job owner's login is required to construct these.  The right
	 * long-term answer is for muskie to stuff this into the initial job
	 * record, in which case we'll always have it.  In the meantime, if we
	 * have no login, we just omit these fields and the zone agent will fail
	 * the task.  This should never actually happen anyway because we
	 * extract the login ourselves from the first input key we get, and we
	 * should always have seen some input key by the time we get here.
	 */
	if (group.g_login) {

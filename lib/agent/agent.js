...
maTaskGroup.prototype.httpState = function ()
{
	var rv = {
	    ...
	    'taskGroupResults': this.t_results
	};
...
/*
 * Prepares the hyprlofs mount for the first input key.
 */
function maTaskGroupSetupHyprlofs(taskgroup, callback)
{
	taskgroup.t_log.info('setting up hyprlofs mount');
	callback();
}
 ...
 * group to the next key.
 */
function maTaskGroupAdvance(taskgroup)
{
	var i = taskgroup.t_results.length;

	if (i < taskgroup.t_input.length) {
		taskgroup.t_current_start = new Date();
		taskgroup.t_current_input = taskgroup.t_input[i];
...
function maTaskGroupMarkDone(taskgroup)
{
	...
	mod_assert.ok(taskgroup.t_failmsg ||
...
{
	var args, error, taskgroup, taskargs;
	...
	taskargs['taskGroupId'] = mod_uuid.v4();
...
function maTaskApiCommit(request, response, next)
{
	...
	/* XXX Should we just remove "key" from the API? */
	...
	taskgroup.t_results.push({
	    'result': 'success',
	    'inputKey': taskgroup.t_current_input,
	    'outputs': taskgroup.t_current_outputs,
	    'start': taskgroup.t_current_start,
	    'done': new Date()
	});
...
function maTaskApiManta(request, response, next)
{
	...
	proxyargs['continue'] = function () {
		if (!taskgroup.t_current_seen.hasOwnProperty(path)) {
			taskgroup.t_current_seen[path] = true;
			taskgroup.t_current_outputs.push(path);

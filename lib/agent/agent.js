...
/*
 * XXX MANTA-928
 * o don't write final task record (in taskMarkOk) until all taskoutput
 *   records have been successfully written
 * o hysteresis for log messages (see TODOs)
 */
...
mAgent.prototype.tick = function ()
{
	...
	this.ma_bus.poll(this.ma_tick_start.getTime());
...
mAgent.prototype.onRecord = function (record, barrier)
{
	...
	schema = mod_schema.sBktJsonSchemas[this.ma_buckets[record['bucket']]];
	error = mod_jsprim.validateJsonObject(schema, record['value']);
	if (error) {
		/* TODO how do we avoid seeing this record every time? */
		this.ma_log.warn(error, 'onRecord: validation error', record);
		return;
	}
...
mAgent.prototype.onRecordTask = function (record, barrier)
{
	...
	if (!this.ma_tasks.hasOwnProperty(taskid)) {
		/* TODO ignore dups */
		this.ma_log.error('onRecord: ignoring record for unknown task',
		    record);
		return;
	}
	...
	if (task.t_ninputs !== undefined ||
		...
		/* TODO ignore subsequent */
		this.ma_log.error('onRecord: unknown reason for match', record);
		return;
	}
...
mAgent.prototype.onRecordTaskInput = function (record, barrier)
{
	...
	if (!this.ma_tasks.hasOwnProperty(taskid)) {
		/* TODO ignore subsequent */
		this.ma_log.error('onRecord: dropping taskinput for unknown ' +
		    'task', record);
		return;
	}
...
mAgent.prototype.jobCreate = function (jobid)
{
	...
	this.ma_bus.oneshot(bucket, query, maMorayOptions, function (record) {
		job.j_record = record;
...
mAgent.prototype.taskMarkFailed = function (task, when, error, barrier)
{
	...
	if (error !== undefined) {
		...
		errvalue = {
		    ...
		    'input': task.t_record['value']['key'],
		    'p0input': task.t_record['value']['p0key'],
...
mAgent.prototype.taskStreamAdvance = function (stream, callback)
{
	...
	localkeypath = taskvalue['key'].substr(1);

 ...
 */
var maPollConfig = [
    { 'name': 'job',		'cache': true,	'mkfilter': pfJob },
    { 'name': 'task',		'cache': true,	'mkfilter': pfTask },
    { 'name': 'taskinput',	'cache': true,	'mkfilter': pfTaskInput }
];
...
function pfJob()
{
	...
	var rv = '|';
	mod_jsprim.forEachKey(maJobs,
	    function (jobid) { rv += '(jobId=' + jobid + ')'; });
	return (rv);
}

function pfTask()
{
	var rv = '&(server=' + maServerName +')' +
	    '(!(state=done))' + '(!(state=aborted))';

	/*
	 * XXX This addition only matters once we also use mtime or bucket
	 * change number to make sure we catch only changes.  We don't care
	 * about tasks that are cancelled when we start up, but we do care about
	 * changes to tasks that were previously not cancelled but which now
	 * are.
	 */
...
function maInitMoray()
{
	...
	maPollConfig.forEach(function (pollconf) {
		maMorayPoller.addPoll({
		    'name': pollconf['name'],
		    'bucket': maConf['buckets'][pollconf['name']],
		    'cache': pollconf['cache'],
...
function maJobUpdated(job, record)
{
	...
	if (record['value']['timeCancelled']) {
		...
		maMorayPoller.remove(maConf['buckets']['job'], job.j_id);
		return;
	}
...
function maTaskCreate(taskid, record)
{
	...
	group.g_log.debug('created group', task.t_id);
...
function maTaskUpdated(task, record)
{
	...
	if (record['value']['state'] == 'cancelled') {
		...
		mod_assert.ok(i < group.g_tasks.length);
		group.g_tasks.splice(i, 1);
		group.g_log.info('task "%s": removed from queue', task.t_id);
		return;
	}

	if (record['value']['timeInputDone']) {
		maLog.debug('task "%s": input done', task.t_id);
		task.t_input_done = record['value']['timeInputDone'];
...
function maTaskRemove(task)
{
	...
	maMorayPoller.remove(maConf['buckets']['task'], task.t_id);
	/* XXX remove associated taskinput records */
}
...
function maTaskAppendKey(task, keyinfo)
{
	...
	if (task.t_group.g_state == maTaskGroup.TASKGROUP_S_RUNNING &&
...
function maTaskDone(task, machine, when)
{
 ...
 */
function maMorayOnRecord(record, haschanged)
{
...
function maSaveTasks(client)
{
	...
	mod_jsprim.forEachKey(maJobs, function (jobid, job) {
		if (!jobs.hasOwnProperty(jobid)) {
			for (var groupid in job.j_groups) {
				var group = maTaskGroups[groupid];
				if (group.g_task)
					tasks.push(group.g_task);
				else if (group.g_tasks.length > 0)
					tasks.push(group.g_tasks[0]);
				break;
			}
		}
	});
	 ...
	 * in order to throttle writes.  As it is, we may end up doing multiple
	 * PUTs for the same record and putting out large numbers of requests.
	 */
	tasks.forEach(function (task) {
		task.t_save.saveStart();
		client.putObject(task.t_record['bucket'], task.t_id,
		    task.t_record, { 'etag': task.t_record['_etag'] },
		    function (err) {
			if (err) {
...
{
	mod_assert.equal(group.g_failmsg !== undefined);
...
function maTaskGroup(groupid, job, pi)
{
	...
	this.g_log = maLog.child({ 'group': groupid });
...
maTaskGroup.prototype.taskState = function ()
{
	...
	if (this.g_multikey) {
		rv['taskInputKeys'] = this.g_task.t_xinput.slice(0, 10);
		rv['taskInputDone'] = this.g_task.t_input_done ? true : false;
...
function maTaskGroupDispatch(group, callback)
{
	...
	group.g_log.info('dispatching taskgroup');
...
function maTaskGroupAdvance(group, callback)
{
	...
	task.t_record['timeStarted'] = mod_jsprim.iso8601(now);
...
function maTaskGroupRemove(group)
{
	...
	if (mod_jsprim.isEmpty(job.j_groups)) {
		delete (maJobs[job.j_id]);
		maMorayPoller.remove(maConf['buckets']['job'], job.j_id);
	}
...
function maTaskGroupMarkDone(group)
{
	...
	zone = maZones[group.g_machine];
	mod_assert.equal(zone.z_group, group);
	mod_assert.equal(zone.z_state, mod_agent_zone.maZone.ZONE_S_BUSY);
	zone.z_group = undefined;
...
function maTaskApiTask(request, response, next)
{
	...
	if (zone.z_taskgroup &&
		...
		if (!group.g_multikey || (group.g_task &&

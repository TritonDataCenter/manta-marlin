...
/*
 * Finds the list of compute zones from vmadm(1).
 */
function maInitZones()
{
	var cmd = 'vmadm lookup tags.manta_role=marlincompute';

	maLog.info('listing zones with "%s"', cmd);

	mod_child.exec(cmd, function (err, stdout, stderr) {
		if (err) {
			maLog.fatal(err, 'failed to read list of zones: %s',
			    stderr);
			throw (err);
		}
 ...
 * EJ_INTERNAL.
 */
function maTaskError(task, nout, when, error)
{
	...
	stream.s_rqqueue.push(function (callback) {
		...
		if (stream.s_task === undefined) {
			...
			if (!skipTaskUpdate &&
				...
				task.t_stream = undefined;
				maTaskError(task, stream.s_noutput, Date.now(),
				    error);
			} else
...
function maTaskStreamAdvance(stream, callback)
{
	...
	if (key['key'][0] != '/') {
		task.t_stream = undefined;
		maTaskError(task, stream.s_noutput, now, {
		    'code': 'EJ_NOENT',
	...
	zone.z_hyprlofs.removeAll(function (err) {
		...
		zone.z_hyprlofs.addMappings([
			...
			task.t_stream = undefined;
			maTaskError(task, stream.s_noutput, new Date(), {
			    'code': 'EJ_NOENT',
			    'message': 'failed to load key',
...
function maTaskApiFail(request, response, next)
{
	...
	stream.s_rqqueue.push(function (callback) {
		...
		task.t_stream = undefined;
		maTaskError(task, stream.s_noutput, now, error);
		maTaskStreamAdvance(stream, callback);
	}, function (sent) {
		if (!sent)
			response.send(204);

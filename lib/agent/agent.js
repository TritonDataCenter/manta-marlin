...
mAgent.prototype.availSlop = function ()
{
	return (this.ma_slopmem - this.ma_slopused);
};
...
mAgent.prototype.zonesCheckLiveness = function ()
{
	...
	mod_jsprim.forEachKey(this.ma_zones, function (zonename, zone) {
		...
		if (now - zone.z_last_contact <= agent.ma_liveness_interval)
			return;
 ...
 */
mAgent.prototype.taskGroupSlop = function (group)
{
...
mAgent.prototype.meter = function (eventname, record)
{
	...
	record['agentUuid'] = this.ma_server_uuid;
...
mAgent.prototype.schedGroupOversched = function (group)
{
	 ...
	 * system.
	 */
	var pool = this.ma_zonepools[group.g_poolid];
	var nzones = this.schedGroupShare(group);
	mod_assert.ok(nzones > 0 && nzones <= pool.nzones());
...
mAgent.prototype.schedIdle = function ()
{
	...
	mod_jsprim.forEachKey(this.ma_zonepools, function (key, pool) {
		var queue, zone, group;

		queue = agent.ma_taskgroups_queued[key];
		...
		while (pool.hasZoneReady() && queue.length > 0) {
			...
			if (agent.taskGroupSlop(group) > agent.availSlop()) {
				group.g_state = maTaskGroup.TASKGROUP_S_INIT;
...
mAgent.prototype.schedPickGroup = function ()
{
	...
	mod_jsprim.forEachKey(agent.ma_taskgroups, function (_, group) {
		...
		if (agent.taskGroupSlop(group) > availslop)
			return;
...
mAgent.prototype.schedGroup = function (group)
{
	...
	if (this.taskGroupSlop(group) > this.availSlop())
		return;

	var zone = this.ma_zones[pool.zonePick()];
...
mAgent.prototype.schedDispatch = function (group, zone)
{
	...
	slop = this.taskGroupSlop(group);
	mod_assert.ok(slop <= this.availSlop());
	...
	this.ma_slopused += slop;

	stream.s_log.info('created stream for job "%s" phase %d (slop %d, ' +
	    'group concurrency now %d)', group.g_jobid, group.g_phasei,
	    slop, group.g_nstreams);
...
mAgent.prototype.zoneReady = function (zone, err, reason)
{
	...
	mod_agent_zone.maZoneSet(zone, options, function (suberr) {
		 ...
		 */
		var giveback = oldmem - options['max_physical_memory'];
		if (giveback > 0) {
			zone.z_log.info('giving back %d MB', giveback);
			mod_assert.ok(giveback <= agent.ma_slopused);
			agent.ma_slopused -= giveback;
		}

		agent.ma_counters['zones_readied']++;
		agent.ma_zonepools[zone.z_image].zoneMarkReady(zone.z_zonename);
		agent.schedIdle();
	});
};
...
function maKangGetObject(type, id)
{
	...
	if (type == 'agent') {
		return ({
		    ...
		    'slopTotal': agent.ma_slopmem,
		    'slopUsed': agent.ma_slopused
		});
	}

...
var mazContractEvent;		/* last contract event */
...
{
	var reduce, keys, key, exec, env, child, done, barrier;
	var cache, infile, instream, outstream, errstream, base;
	...
	base = mod_path.normalize(mod_path.join(__dirname, '../..'));
	...
	env = {
	    ...
	    'MANTA_JOB_ID': mazCurrentTask['jobId']
	};
	...
	if (!reduce) {
		...
		env['MANTA_INPUT_OBJECT'] = key;
	} else {
		env['MANTA_REDUCER'] = mazCurrentTask['rIdx'];
	}
	...
	if (mazCurrentTask.hasOwnProperty('taskInputFile')) {
		 ...
		 */
		if (!mazCurrentTask['taskInputDone'] ||
		    mazCurrentTask['taskInputKeys'].length > 0)
	...
	var onOpen = function () {
		...
		barrier.start('contract');
		barrier.start('child process');
...
function mazTaskReport(err)
{
	...
	/*
	 * Core dumps trump other errors.  If your command is dumping core, fix
	 * that first.
	 */
	if (mazContractEvent['type'] == 'pr_core') {
		mazTaskResult = 'fail';
		mazExecStatus = 'user command or child process dumped core';
	}

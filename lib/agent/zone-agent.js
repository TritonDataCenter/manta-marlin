...
function mazFileSave(name, filename, key, callback)
{
	...
	mod_fs.stat(filename, function (err, stat) {
		...
		instream.on('open', function () {
			opened = true;
			mazStreamSave(name, stat['size'], instream, key,
			    callback);
		});
	});
}

function mazStreamSave(name, size, stream, key, callback, noretry)
{
	var options = { 'size': size };

	if (name == 'stderr')
		options['headers'] = { 'x-marlin-stream': 'stderr' };

	mazMantaClient.put(key, stream, options, function (err) {
		if (!err) {
			mazLog.info('uploaded %s', name);
			callback();
			return;
		}

		if (noretry || err.name != 'DirectoryDoesNotExistError') {
			mazLog.error(err, 'failed to save %s', name);
			callback(err);
			return;
		}

		mazLog.info(err, 'need to create directory');
		mazMantaClient.mkdirp(mod_path.dirname(key), function (err2) {
			if (err2) {
				mazLog.error(err2, 'failed to mkdirp for %s',
				    name);
			...
			mazStreamSave(name, size, stream, key, callback, true);
		});
	});
}

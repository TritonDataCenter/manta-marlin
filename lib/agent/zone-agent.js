...
function mazTaskRunSaveOutput(task, callback)
{
	...
	if (!mazCurrentTask['taskOutputKey']) {
		/*
		 ...
		 */
		mazLog.warn('error picking manta key for stdout: ' +
		    'taskOutputKey not set');
		callback();
		return;
	}
	...
	mod_fs.stat(filename, function (err, stat) {
		...
		instream.on('open', function () {
			...
			opened = true;
			name = mazCurrentTask['taskOutputKey'];

			var uri = mod_path.join('/object', name);
...
function mazApiProxy(request, response, next)
{
	...
	if (request.method.toLowerCase() == 'put' &&
	    mod_jsprim.startsWith(request.url, '/object/'))
		mazUserEmitted = true;

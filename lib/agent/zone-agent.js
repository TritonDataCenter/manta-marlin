...
var mazTaskOutputDir;		/* output directory for reduce keys */
...
function mazTaskRunSpawn(task, callback)
{
	...
	/* XXX see naming below. */
	mazTaskOutputDir = mod_path.dirname(keys[0]);
	...
	var onOpen = function () {
		...
		child = mod_child.spawn('bash', [ '-c', exec ], {
		    'stdio': [ infile ? instream : null, outstream, errstream ],
		    'env': env
		});
			...
			if (mazContract !== undefined) {
				...
				if (!infile) {
					instream.on('error', function (err) {
						mazLog.warn(err, 'error ' +
						    'after closing instream');
					});
	...
	mod_fs.stat(filename, function (err, stat) {
		...
		instream.on('open', function () {
			...
			if (mazCurrentTask['taskPhase']['type'] == 'reduce')
				name = mod_path.join(mazTaskOutputDir,
				    mazCurrentTask['taskId']);
			else
				name = mazCurrentTask['taskInputKeys'][0];
			...
			request.on('response', function (response) {
				mazLog.info('stdout save: response', {
				    'uri': uri,
				    'x-request-id':
				...
				mazLog.error('failed to save stdout: ' +
				    'server returned code %s',
				    response.statusCode);
				...
				response.on('end', function () {
					mazLog.error('response body', data);
					callback(new Error('failed to save ' +
					    'stdout: server returned ' +
					    response.statusCode));
				});
			});
			...
			request.on('error', function (suberr) {
				...
				callback(new mod_verror.VError(suberr,
				    'failed to save stdout: request failed'));
			});
		});
	});
}
...
function mazDoneBatch()
{
	...
	mazParentRequest('POST /commit', 204, function (rqcallback) {
		mazClient.post('/commit', arg, rqcallback);
	}, function (err) {
		if (err) {
		...
		mazParentRequest('GET /task?wait=true', 200,
		    function (subrequestcallback) {
			mazClient.get('/task?wait=true', subrequestcallback);

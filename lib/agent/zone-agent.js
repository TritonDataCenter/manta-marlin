...
function mazParentRequest(opname, expected_code, makerequest, callback)
{
	...
	mod_assert.ok(!mazClientRequests.hasOwnProperty(uuid));
	mazClientRequests[uuid] = { 'time': new Date(), 'opname': opname };
	makerequest(function (err, request, response, result) {
		...
		if (!err) {
			...
			err = new mod_verror.VError(null, 'expected %s ' +
		...
		mazLog.warn(err, '%s failed', opname);
...
function mazApiProxy(request, response, next)
{
	 ...
	 * requests are -- even Manta requests.
	 */

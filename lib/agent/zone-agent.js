...
function mazTaskRun()
{
	...
	child.on('exit', function (code) {
		...
		if (code !== 0) {
			/*
			 * We will eventually support a retry policy, but for
			 * now we just report failure to our parent agent, who
			 * will consider the whole task having failed.
			 * XXX We only really failed this one key.  This request
			 * should indicate which key failed, our parent should
			 * record that, and then we should probably goto top and
			 * try the next key.
			 */
			mazCounters['keys_fail']++;
			mazLog.error('task failed (exit code %s)', code);
			mazParentRequest('POST /fail', 204, function (rqcb) {
				mazClient.post('/fail', rqcb);
			}, function () { mazCurrentKey = undefined; });
			return;
		}
		...
		mazParentRequest('POST /commit', 204, function (rqcallback) {
			mazClient.post('/commit', { 'key': key }, rqcallback);
		}, function () {

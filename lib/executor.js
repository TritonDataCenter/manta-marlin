...
Executor.prototype.spawn = function ()
{
	 ...
	 * socket is no longer writable.  (This is arguably a bug, but we're
	 * talking about a heuristic for determining if bash is running under
	 * rsh.)   Node only shuts down the socket when we call end(), so it's a
	 * race between end() and bash startup.  We avoid this mess by just
	 * specifying --norc.
	 */

...
/*
 * Kill all child processes created by this Executor.  This may only be called
 * after start() and before the 'done' event is emitted.
 */
Executor.prototype.abort = function ()
{
	/*
	 * It would be nice if Node exported these constants from the core
	 * library, but it currently does not.  Note that errno values actually
	 * do vary between systems, so care must be taken in porting this code.
	 * This code all relies on illumos-specific contracts anyway, and
	 * illumos has committed to the stability of these values.
	 */
	var SIGKILL = 9;
	var ESRCH = 3;
	mod_assertplus.ok(this.ex_contract !== undefined);

	try {
		this.ex_contract.sigsend(SIGKILL);
	} catch (ex) {
		if (ex['errno'] == ESRCH) {
			this.ex_log.debug(ex, 'sigsend');
			return;
		}

		this.ex_log.fatal(ex, 'sigsend', ex);
		this.ex_error = ex;
		throw (ex);
	}
};
...
Executor.prototype.onChildExit = function (code, signal)
{
	...
	/*
	 * Kill processes left in the contract once the immediate child exits.
	 */
